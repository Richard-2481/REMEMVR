...
if __name__ == "__main__":
    try:
        log("[START] Step 5: posthoc_contrasts")
        ...
        log(f"[LOADED] {fixed_effects_path.name}")

        # Find interaction p-values
        interaction_terms = fixed_effects[fixed_effects['term'].str.contains('paradigm.*TSVR')]
        if len(interaction_terms) == 0:
            log("[WARNING] No paradigm×TSVR interaction terms found")
            log("[INFO] Skipping post-hoc contrasts (interaction not present)")
            sys.exit(0)

        max_interaction_p = interaction_terms['p_uncorrected'].max()
        log(f"[INFO] Paradigm×Time interaction max p-value: {max_interaction_p:.4f}")

        if max_interaction_p >= 0.05:
            log("[INFO] Interaction not significant (p >= 0.05), skipping post-hoc tests")
            ...
            log(f"[SAVED] {out_path.name} (placeholder - no contrasts computed)")
            sys.exit(0)

        log("[INFO] Interaction significant, proceeding with post-hoc contrasts")
        log("[NOTE] Requires fitted LMM model object - placeholder implementation")
        ...
        log(f"[SAVED] {out_path.name} (placeholder)")
        ...
        if validation_result.get('d068_compliant', False):
            log("[PASS] D068 dual p-value compliance validated")
        else:
            log(f"[WARNING] D068 validation: {validation_result.get('message', 'Unknown')}")

        log("[SUCCESS] Step 5 complete (placeholder)")
        sys.exit(0)
