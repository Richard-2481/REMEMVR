"""
RQ 6.4.1 - Step 05: Fit Extended LMM Model Suite (Kitchen Sink) with Paradigm Interaction

PURPOSE:
Compare 70+ LMM trajectory models to identify best functional form for
paradigm-specific (IFR/ICR/IRE) confidence decline trajectories.

INPUT:
- data/step04_lmm_input.csv (1200 rows = 400 observations × 3 paradigms)

INTERACTION:
- factor1: paradigm (categorical: IFR, ICR, IRE)
- Tests time×paradigm interaction effects
...
if __name__ == "__main__":
    try:
        log("=" * 80)
        log("[START] Step 05: Kitchen Sink LMM - Paradigm Interaction")
        log("=" * 80)

        log("[LOAD] Loading LMM input data...")
        lmm_input = pd.read_csv(DATA_DIR / "step04_lmm_input.csv", encoding='utf-8')
        log(f"  ✓ Loaded {len(lmm_input)} rows ({lmm_input['UID'].nunique()} participants × {lmm_input['paradigm'].nunique()} paradigms)")
        log(f"  ✓ Paradigms: {sorted(lmm_input['paradigm'].unique())}")

        log("[ANALYSIS] Running kitchen sink with paradigm interaction...")
        results = compare_lmm_models_kitchen_sink(
            ...
            factor1_reference='IFR',  # Reference level
            factor2_var=None,
