...
RQ_DIR = Path(__file__).resolve().parents[1]  # results/ch6/6.4.1 (derived from script location)
...
if __name__ == "__main__":
    try:
        ...
        # Clean paradigm names (remove "theta_" prefix)
        df_long['paradigm'] = df_long['paradigm'].str.replace('theta_', '')
        log(f"Paradigms: {df_long['paradigm'].unique().tolist()}")
        ...
        df_long = df_long[[
            'composite_ID', 'UID', 'test', 'TSVR_hours', 'log_TSVR', 'paradigm', 'theta'
        ]]
        ...
        # Check 6: Paradigm values correct
        expected_paradigms = {'IFR', 'ICR', 'IRE'}
        actual_paradigms = set(df_long['paradigm'].unique())
        if actual_paradigms == expected_paradigms:
            log(f"Paradigms correct: {sorted(actual_paradigms)}")
        else:
            log(f"Paradigm mismatch - Expected: {expected_paradigms}, Got: {actual_paradigms}")
            sys.exit(1)

        # Summary statistics
        log("Summary statistics by domain:")
        for domain in sorted(df_long['paradigm'].unique()):
            domain_data = df_long[df_long['paradigm'] == domain]
