...
def compute_slope_contrasts(model, domain_levels):
    ...
    - C(paradigm)[T.When]:log_TSVR: difference from What slope
    - C(paradigm)[T.Where]:log_TSVR: difference from What slope
    ...
    # Extract interaction terms (6.4.1 uses IFR/ICR/IRE paradigms, ICR is reference)
    beta_IFR_TSVR = params['C(paradigm)[T.IFR]:log_TSVR']
    beta_IRE_TSVR = params['C(paradigm)[T.IRE]:log_TSVR']

    # Contrast 1: IFR vs ICR (already in model as interaction)
    contrast1_est = beta_IFR_TSVR
    contrast1_se = np.sqrt(cov.loc['C(paradigm)[T.IFR]:log_TSVR', 'C(paradigm)[T.IFR]:log_TSVR'])

    # Contrast 2: IRE vs ICR (already in model as interaction)
    contrast2_est = beta_IRE_TSVR
    contrast2_se = np.sqrt(cov.loc['C(paradigm)[T.IRE]:log_TSVR', 'C(paradigm)[T.IRE]:log_TSVR'])

    # Contrast 3: IFR vs IRE (difference of interactions)
    contrast3_est = beta_IFR_TSVR - beta_IRE_TSVR

    # Variance for difference: Var(A - B) = Var(A) + Var(B) - 2*Cov(A,B)
    var_IFR = cov.loc['C(paradigm)[T.IFR]:log_TSVR', 'C(paradigm)[T.IFR]:log_TSVR']
    var_IRE = cov.loc['C(paradigm)[T.IRE]:log_TSVR', 'C(paradigm)[T.IRE]:log_TSVR']
    cov_IFR_IRE = cov.loc['C(paradigm)[T.IFR]:log_TSVR', 'C(paradigm)[T.IRE]:log_TSVR']
    contrast3_se = np.sqrt(var_IFR + var_IRE - 2*cov_IFR_IRE)

    # Compute z-statistics and p-values
    contrasts = []

    for name, est, se in [
        ('IFR vs ICR', contrast1_est, contrast1_se),
        ('IRE vs ICR', contrast2_est, contrast2_se),
        ('IFR vs IRE', contrast3_est, contrast3_se)
    ]:
        z = est / se
    ...
    try:
        ...
        if min_interaction_p < 0.05:
            ...
            with warnings.catch_warnings():
                ...
                model = smf.mixedlm(
                    formula="theta ~ C(paradigm) * log_TSVR",
                    data=df_lmm,
                    groups=df_lmm["UID"],
                    re_formula="~log_TSVR"
