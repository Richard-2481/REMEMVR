...
if __name__ == "__main__":
    try:
        ...
        log(f"Congruences: {df_long['location'].unique().tolist()}")
        ...
        if len(df_long) == expected_rows:
            log(f"Row count: {len(df_long)} rows = {len(df_theta)} theta rows * 2 locations")
        else:
        ...
        if actual_locations == expected_locations:
            log(f"Locations correct: {sorted(actual_locations)}")
        else:
            log(f"Location mismatch - Expected: {expected_locations}, Got: {actual_locations}")
            sys.exit(1)

        # Summary statistics
        log("Summary statistics by location:")
        for location in sorted([loc for loc in df_long['location'].unique() if loc != 'UID']):
            ...
            log(f"{location}: N={n_obs}, theta mean={theta_mean:.3f}, SD={theta_std:.3f}")
