...
Uses same 2-factor structure (Source/Destination) but only high-quality items.
...
EXPECTED INPUTS:
  ...
  - data/step00_q_matrix.csv
    ...
    Format: Q-matrix for 2-factor structure
...
g_code REASONING:
- Approach: Same 2-factor GRM as Pass 1, but only on purified items
...
- Parameters: 2 factors (Source/Destination), 5 categories, correlated factors, cpu device
...
if __name__ == "__main__":
    try:
        log("[START] Step 03: IRT Calibration Pass 2 (2-factor GRM on purified items only)")
        log(f"[CONFIG] Model: 2-factor GRM, {IRT_CONFIG['n_cats']} categories")
        ...
        if empty_domains:
            ...
            log(f"[INFO] All 2 factors active for Pass 2: {active_factors}")
