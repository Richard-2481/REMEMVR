"""
RQ 6.8.1 - Step 05: Fit Extended LMM Model Suite (Kitchen Sink) with Location Interaction

PURPOSE:
Compare 70+ LMM trajectory models to identify best functional form for
location-specific (Source/Destination) confidence decline trajectories.

INPUT:
- data/step04_lmm_input.csv (800 rows = 400 observations × 2 locations)

INTERACTION:
- factor1: location (categorical: Source, Destination)
- Tests time×location interaction effects
...
if __name__ == "__main__":
    try:
        ...
        results = compare_lmm_models_kitchen_sink(
            ...
            tsvr_var='TSVR_hours',
            groups_var='UID',
            factor1_var='location',
            factor1_type='categorical',
            factor1_reference='Source',  # Reference level
            factor2_var=None,
