...
def compute_slope_contrasts(model, domain_levels):
    ...
    - C(location)[T.When]:log_TSVR: difference from What slope
    - C(location)[T.Where]:log_TSVR: difference from What slope
    ...
    beta_When_TSVR = params['C(location)[T.When]:log_TSVR']
    beta_Where_TSVR = params['C(location)[T.Where]:log_TSVR']

    # Contrast 1: When vs What (already in model as interaction)
    contrast1_est = beta_When_TSVR
    contrast1_se = np.sqrt(cov.loc['C(location)[T.When]:log_TSVR', 'C(location)[T.When]:log_TSVR'])

    # Contrast 2: Where vs What (already in model as interaction)
    contrast2_est = beta_Where_TSVR
    contrast2_se = np.sqrt(cov.loc['C(location)[T.Where]:log_TSVR', 'C(location)[T.Where]:log_TSVR'])
    ...
    var_When = cov.loc['C(location)[T.When]:log_TSVR', 'C(location)[T.When]:log_TSVR']
    var_Where = cov.loc['C(location)[T.Where]:log_TSVR', 'C(location)[T.Where]:log_TSVR']
    cov_When_Where = cov.loc['C(location)[T.When]:log_TSVR', 'C(location)[T.Where]:log_TSVR']
...
def main():
    ...
    try:
        ...
        if min_interaction_p < 0.05:
            ...
            log("      Formula: theta ~ C(location) * log_TSVR", logger)
