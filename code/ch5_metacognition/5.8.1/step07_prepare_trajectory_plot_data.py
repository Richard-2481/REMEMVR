...
RQ_DIR = Path(__file__).resolve().parents[1]  # results/ch6/6.8.1
...
if __name__ == "__main__":
    try:
        ...
        grouped.columns = ['location', 'TEST', 'theta', 'se', 'n', 'time']
        ...
        theta_data = grouped[['time', 'theta', 'CI_lower', 'CI_upper', 'location', 'n']].copy()

        # Sort by domain, then time
        theta_data = theta_data.sort_values(['location', 'time']).reset_index(drop=True)

        log(f"Aggregated to {len(theta_data)} rows (domains × tests)")
        log(f"Domains present: {sorted(theta_data['location'].unique())}")
        log(f"Tests per domain: {theta_data.groupby('location').size().to_dict()}")
        ...
        # Columns: ['time', 'theta', 'CI_lower', 'CI_upper', 'location', 'n']
        ...
        for location in locations:
            discrim_col = f'Discrim_{location}'
            ...
            if discrim_col in df_items.columns:
                ...
                if len(domain_items) > 0:
                    # Compute mean discrimination for this location
                    mean_discrim[location] = domain_items[discrim_col].mean()
                    log(f"Mean discrimination for {location}: {mean_discrim[location]:.4f} ({len(domain_items)} items)")
                else:
                    log(f"No items found for {location} location (location excluded after purification)")
            else:
        ...
        for domain in probability_data['location'].unique():
            ...
            domain_mask = probability_data['location'] == domain
        ...
        probability_data = probability_data[['time', 'probability', 'CI_lower', 'CI_upper', 'location', 'n']]
        ...
        # Columns: ['time', 'probability', 'CI_lower', 'CI_upper', 'location', 'n']
        ...
        n_domains = len(theta_data['location'].unique())
