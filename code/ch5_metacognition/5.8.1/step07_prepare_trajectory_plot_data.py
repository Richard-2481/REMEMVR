...
if __name__ == "__main__":
    try:
        ...
        log(f"[INFO] Domains present: {sorted(theta_data['location'].unique())}")
        log(f"[INFO] Tests per domain: {theta_data.groupby('location').size().to_dict()}")
        ...
        for location in locations:
            ...
            if discrim_col in df_items.columns:
                ...
                if len(domain_items) > 0:
                    ...
                    log(f"[INFO] Mean discrimination for {location}: {mean_discrim[location]:.4f} ({len(domain_items)} items)")
                else:
                    log(f"[WARNING] No items found for {location} location (location excluded after purification)")
            else:
