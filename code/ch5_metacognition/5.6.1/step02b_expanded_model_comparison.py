#!/usr/bin/env python3
"""EXPANDED AIC Model Comparison with Power-Law and Advanced Transformations: Comprehensive functional form testing including power-law transformations, piecewise"""
...
if __name__ == "__main__":
    try:
        log("Step 02b: EXPANDED AIC Model Comparison with Power-Law Transformations")
        # Load Input Data
        ...
        # Create EXPANDED Time Transformations
        #          exponential decay
        #
        # Basic (from Step 02a):
        #   1. Days = TSVR / 24
        ...
        log("Creating EXPANDED time transformations from TSVR...")
        ...
        log("\nFitting 13 candidate LMM models with REML=False...")
        ...
        if len(converged_models) == 0:
            ...
            log(f"\n{best_model_name} (weight={best_weight:.4f}, AIC={best_aic:.2f})")
        ...
        if len(top5_models) > 0:
            ...
            if best_model_name == 'Piecewise_Day1':
                ...
                log("Piecewise model wins - two-phase pattern supported by data")
            ...
            if best_model_name in power_models_lt_1:
                ...
                log(f"Power-law < 1.0 model ({best_model_name}) wins - diminishing returns pattern")
            ...
            if len(top5_models) >= 2:
                ...
                if delta_aic_best_vs_second > 2.0:
                    ...
                    log(f"Clear winner - ΔAIC={delta_aic_best_vs_second:.2f} > 2")
                else:
                    flags.append(f"COMPETITIVE_MODELS: ΔAIC={delta_aic_best_vs_second:.2f} < 2 (top models competitive)")
                    log(f"Competitive models - ΔAIC={delta_aic_best_vs_second:.2f} < 2")

        if len(flags) == 0:
            flags.append("No special patterns detected")
            log("No special patterns detected")
        # Save Model Comparison Results

        log("\nSaving expanded model comparison results...")
        ...
        if len(df_comparison) != 13:
            log(f"Expected 13 models, found {len(df_comparison)}")
            validation_passed = False
        else:
            log("All 13 models attempted")
        ...
        if finite_aic_count == 0:
            ...
            log(f"{finite_aic_count}/13 models have finite AIC values")
        ...
        if len(top5_models) == 0:
            log("No top 5 models ranked (no converged models)")
            validation_passed = False
        elif len(top5_models) < 5 and len(converged_models) >= 5:
            log(f"Only {len(top5_models)} models in top 5 (expected 5)")
        else:
            log(f"Top {len(top5_models)} models ranked")
        ...
        log(f"{len(flags)} special pattern flags documented")
        ...
        if validation_passed:
            log("\nStep 02b complete - all validations passed")
            sys.exit(0)
        else:
            log("\nStep 02b complete with validation warnings")
