...
# Congruence Assignment Function

def assign_congruence_from_tag(item_name: str) -> str:
    """
    Parse congruence category from TC_* column name based on i-tag.

    Congruence categories:
      - Common: i1, i2 (everyday objects)
      - Congruent: i3, i4 (schema-consistent placements)
      - Incongruent: i5, i6 (schema-violating placements)
    ...
    Raises:
        ValueError if i-tag not recognized
    """
    if '-i1' in item_name or '-i2' in item_name:
        return "common"
    elif '-i3' in item_name or '-i4' in item_name:
        return "congruent"
    elif '-i5' in item_name or '-i6' in item_name:
        return "incongruent"
    else:
        raise ValueError(f"Cannot determine congruence for item: {item_name}")

# Main Analysis

if __name__ == "__main__":
    try:
        log("Step 00: Extract Confidence Data (Congruence-Based)")
        ...
        # Create Q-Matrix (3-Factor Structure BY CONGRUENCE)
        # Assign each TC_* item to congruence category based on i-tag
        # Q-matrix encoding: Common (i1/i2), Congruent (i3/i4), Incongruent (i5/i6)
        #
        # KEY DIFFERENCE from RQ 6.3.1 and 6.4.1:
        # RQ 6.3.1: Q-matrix by domain (What/Where/When)
        # RQ 6.4.1: Q-matrix by paradigm (IFR/ICR/IRE)
        # RQ 6.5.1: Q-matrix by congruence (Common/Congruent/Incongruent)

        log("Creating 3-factor Q-matrix BY CONGRUENCE...")

        q_matrix_data = []
        for item in tc_interactive:
            congruence = assign_congruence_from_tag(item)

            # Create binary loadings for 3-factor structure
            # Each item loads on exactly ONE factor (its congruence category)
            q_matrix_data.append({
                'item_name': item,
                'factor_common': 1 if congruence == 'common' else 0,
                'factor_congruent': 1 if congruence == 'congruent' else 0,
                'factor_incongruent': 1 if congruence == 'incongruent' else 0
            })

        df_q_matrix = pd.DataFrame(q_matrix_data)

        log(f"Q-matrix: {len(df_q_matrix)} items")
        log(f"Common (i1/i2): {df_q_matrix['factor_common'].sum()} items")
        log(f"Congruent (i3/i4): {df_q_matrix['factor_congruent'].sum()} items")
        log(f"Incongruent (i5/i6): {df_q_matrix['factor_incongruent'].sum()} items")
        # Save Outputs
        ...
        # Check 4: All 3 congruence categories represented in Q-matrix
        common_count = df_q_matrix['factor_common'].sum()
        congruent_count = df_q_matrix['factor_congruent'].sum()
        incongruent_count = df_q_matrix['factor_incongruent'].sum()

        if common_count == 0 or congruent_count == 0 or incongruent_count == 0:
            log(f"Q-matrix missing congruence category: Common={common_count}, Congruent={congruent_count}, Incongruent={incongruent_count}")
            validation_passed = False
        else:
            log(f"Q-matrix has all 3 congruence categories (Common={common_count}, Congruent={congruent_count}, Incongruent={incongruent_count})")

        # Check 5: Each item loads on exactly ONE factor
        row_sums = df_q_matrix[['factor_common', 'factor_congruent', 'factor_incongruent']].sum(axis=1)
