"""
RQ 6.5.1 - Step 05b: Model Averaging for Schema-Congruent Confidence Trajectories

PURPOSE:
Implement Burnham & Anderson (2002) model averaging for schema congruence
(Common/Unique) confidence trajectories with interaction terms.

INPUT:
- data/step04_lmm_input.csv (800 rows: 100 participants × 4 tests × 2 congruence levels)
- data/step05_model_comparison.csv

INTERACTION FACTOR: congruence (Common, Unique)
...
if __name__ == "__main__":
    try:
        log("=" * 80)
        log("[START] Step 05b: Model Averaging for RQ 6.5.1 (Schema Congruence)")
        log("=" * 80)

        lmm_input = pd.read_csv(DATA_DIR / "step04_lmm_input.csv")
        comparison = pd.read_csv(DATA_DIR / "step05_model_comparison.csv")

        log(f"\n[LOAD] LMM input: {len(lmm_input)} rows, {lmm_input['congruence'].nunique()} congruence levels")
