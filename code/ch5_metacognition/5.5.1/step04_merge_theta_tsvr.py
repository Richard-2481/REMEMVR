...
if __name__ == "__main__":
    try:
        ...
        log(f"Congruences: {df_long['congruence'].unique().tolist()}")
        ...
        if actual_congruences == expected_congruences:
            log(f"Congruences correct: {sorted(actual_congruences)}")
        else:
            log(f"Congruence mismatch - Expected: {expected_congruences}, Got: {actual_congruences}")
            sys.exit(1)
