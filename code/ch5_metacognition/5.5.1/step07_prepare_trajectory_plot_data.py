...
if __name__ == "__main__":
    try:
        ...
        grouped = df_long.groupby(['congruence', 'test']).agg({
            'theta': ['mean', 'sem', 'count'],
            'TSVR_hours': 'mean'  # Average time per test
        }).reset_index()

        # Flatten column names
        grouped.columns = ['congruence', 'test', 'theta', 'se', 'n', 'time']
        ...
        theta_data = grouped[['time', 'theta', 'CI_lower', 'CI_upper', 'congruence', 'n']].copy()

        # Sort by domain, then time
        theta_data = theta_data.sort_values(['congruence', 'time']).reset_index(drop=True)

        log(f"Aggregated to {len(theta_data)} rows (domains × tests)")
        log(f"Domains present: {sorted(theta_data['congruence'].unique())}")
        log(f"Tests per domain: {theta_data.groupby('congruence').size().to_dict()}")
        ...
        # Columns: ['time', 'theta', 'CI_lower', 'CI_upper', 'congruence', 'n']
        ...
        # Discrim_Common, Discrim_Congruent, Discrim_Incongruent
        ...
        for congruence in ['Common', 'Congruent', 'Incongruent']:
            discrim_col = f'Discrim_{congruence}'
            ...
            if discrim_col in df_items.columns:
                ...
                if len(domain_items) > 0:
                    # Compute mean discrimination for this congruence
                    mean_discrim[congruence] = domain_items[discrim_col].mean()
                    log(f"Mean discrimination for {congruence}: {mean_discrim[congruence]:.4f} ({len(domain_items)} items)")
                else:
                    log(f"No items found for {congruence} congruence (congruence excluded after purification)")
            else:
        ...
        for domain in probability_data['congruence'].unique():
            ...
            domain_mask = probability_data['congruence'] == domain
        ...
        probability_data = probability_data[['time', 'probability', 'CI_lower', 'CI_upper', 'congruence', 'n']]
        ...
        # Columns: ['time', 'probability', 'CI_lower', 'CI_upper', 'congruence', 'n']
        ...
        n_domains = len(theta_data['congruence'].unique())
