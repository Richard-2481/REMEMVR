...
- data/step03_pass2_item_params.csv
  Columns: ['item_name', 'Difficulty', 'Overall_Discrimination', 'Discrim_Common', 'Discrim_Congruent', 'Discrim_Incongruent']
...
if __name__ == "__main__":
    try:
        ...
        log(f"[INFO] Domains present: {sorted(theta_data['congruence'].unique())}")
        log(f"[INFO] Tests per domain: {theta_data.groupby('congruence').size().to_dict()}")
        ...
        for congruence in ['Common', 'Congruent', 'Incongruent']:
            ...
            if discrim_col in df_items.columns:
                ...
                if len(domain_items) > 0:
                    ...
                    log(f"[INFO] Mean discrimination for {congruence}: {mean_discrim[congruence]:.4f} ({len(domain_items)} items)")
                else:
                    log(f"[WARNING] No items found for {congruence} congruence (congruence excluded after purification)")
            else:
