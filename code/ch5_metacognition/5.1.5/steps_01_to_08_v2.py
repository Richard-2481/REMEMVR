#!/usr/bin/env python3
"""
RQ 6.1.5: Trajectory Clustering - Confidence Phenotypes (V2)
============================================================

REVISION: Use K=3 to match Ch5 5.1.5 for valid cross-RQ comparison.
BIC monotonically decreases (known issue), so we match Ch5's K for comparability.
...
Steps:
  ...
  03. K-means clustering for K=2-6 with BIC + elbow analysis
  04. Fit final K-means model with K=3 (matched to Ch5 5.1.5)
...
# Parameters
K_RANGE = range(2, 7)  # K=2 to K=6 (K=1 is trivial)
RANDOM_STATE = 42
N_BOOTSTRAP = 1000
...
# CRITICAL: Match Ch5 5.1.5's K=3 for valid cross-RQ comparison
...
def step02_standardize_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if outliers_intercept > 0 or outliers_slope > 0:
        ...
        # Identify outlier UIDs
        outlier_mask = (abs(df['intercept_z']) > 3) | (abs(df['slope_z']) > 3)
        outlier_uids = df.loc[outlier_mask, 'UID'].tolist()
...
def step03_cluster_selection(df: pd.DataFrame) -> int:
    ...
    log("")
    log("=" * 60)
    log("STEP 03: K-Means Clustering for K=2-6 with BIC Analysis")
    log("=" * 60)
    ...
    for k in K_RANGE:
        ...
        log(f"K={k}: SSE={sse:.4f}, BIC={bic:.4f}, silhouette={sil:.4f}")

    results_df = pd.DataFrame(results)

    # Find BIC minimum (for documentation)
    bic_optimal_k = results_df.loc[results_df['BIC'].idxmin(), 'K']
    log(f"")
    log(f"BIC minimum at K={bic_optimal_k}")
    log(f"NOTE: BIC monotonically decreases - not reliable for this data")

    # Find silhouette maximum
    sil_optimal_k = results_df.loc[results_df['silhouette'].idxmax(), 'K']
    log(f"Silhouette maximum at K={sil_optimal_k}")

    # Use FORCED_K for comparability with Ch5 5.1.5
    log(f"")
    log(f"DECISION: Using K={FORCED_K} to match Ch5 5.1.5 for valid cross-RQ comparison")
    log(f"  Ch5 5.1.5 used K=3 (cluster sizes: 25, 44, 31)")
    log(f"  This enables meaningful chi-square test of phenotype association")

    results_df['optimal'] = results_df['K'] == FORCED_K
    ...
    log(f"")
    log(f"Selected K: {FORCED_K} (matched to Ch5 5.1.5)")
    log("VALIDATION PASS: K selection complete")
...
def step04_fit_final_kmeans(df: pd.DataFrame, optimal_k: int) -> pd.DataFrame:
    ...
    if min_size < MIN_CLUSTER_SIZE:
        log(f"WARNING: Small cluster detected (min size = {min_size}, threshold = {MIN_CLUSTER_SIZE})")
    else:
        log(f"All clusters meet minimum size threshold (>= {MIN_CLUSTER_SIZE})")
    ...
    log(f"VALIDATION PASS: All participants assigned to clusters")
...
def step07_crosstab_clusters(df: pd.DataFrame) -> pd.DataFrame:
    ...
    log(f"Ch5 5.1.5 cluster sizes: {df_accuracy['cluster_accuracy'].value_counts().to_dict()}")
...
def step08_chi_square_test(crosstab: pd.DataFrame):
    ...
    log(f"  V < 0.10: negligible, 0.10-0.30: small, 0.30-0.50: medium, > 0.50: large")
...
def main():
    ...
    log("RQ 6.1.5: Trajectory Clustering - Confidence Phenotypes (V2)")
    log("=" * 60)
    log(f"Project root: {PROJECT_ROOT}")
    log(f"RQ directory: {RQ_DIR}")
    log(f"FORCED_K: {FORCED_K} (matched to Ch5 5.1.5)")
    log("")
    ...
    try:
        ...
        # Step 03: K-means clustering with BIC analysis
        ...
        log("=" * 60)
        log(f"K: {optimal_k} (matched to Ch5 5.1.5)")
        log(f"Phenotypes identified: {list(cluster_stats['phenotype'].values)}")
        log(f"Cluster sizes: {list(cluster_stats['N'].values)}")
