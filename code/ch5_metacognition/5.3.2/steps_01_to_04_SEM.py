#!/usr/bin/env python3
"""
RQ 6.3.2: Domain Confidence Calibration (SEM VERSION)
======================================================
Tests whether calibration quality differs across episodic memory domains (What/Where/When)
using SEM latent calibration scores (measurement error corrected).

**TIER 1 SEM VALIDATION** - Crossover interaction at risk (χ²=59.60, p<0.0001)

Steps:
  01: Fit LMM with Domain × Time interaction (using latent_calibration from SEM)
  02: Compute post-hoc pairwise domain contrasts
  03: Rank domains by calibration quality (|latent_calibration|)
  04: Prepare calibration trajectory plot data

Dependencies:
  - step05_calibration_scores_SEM.csv (SEM latent calibration by domain, 1200 rows)
  - step00_tsvr_mapping from 6.3.1 (inherited via step00_calibration_by_domain.csv)
"""
...
LOG_FILE = RQ_DIR / "logs" / "steps_01_to_04_SEM.log"
DATA_DIR = RQ_DIR / "data"

# Source file (SEM latent calibration)
SEM_CALIBRATION_FILE = DATA_DIR / "step05_calibration_scores_SEM.csv"
...
# ============================================================================
# STEP 00: LOAD SEM CALIBRATION DATA
# ============================================================================

def step00_load_sem_calibration() -> pd.DataFrame:
    """
    Load SEM latent calibration scores by domain (output from step05_compute_calibration_SEM.py).
    """
    log_section("STEP 00: Load SEM Latent Calibration Data")

    # Load pre-computed SEM calibration scores
    log(f"Loading SEM calibration from: {SEM_CALIBRATION_FILE}")
    ...
    log(f"  Domains: {df['Domain'].unique().tolist()}")
    log(f"  UIDs: {df['UID'].nunique()}")

    # Rename TEST to test for consistency if needed
    if 'TEST' in df.columns and 'test' not in df.columns:
        df = df.rename(columns={'TEST': 'test'})

    log(f"  Latent calibration mean: {df['latent_calibration'].mean():.4f}")
    log(f"  Latent calibration std: {df['latent_calibration'].std():.4f}")

    # Create abs_calibration for ranking analysis
    df['abs_calibration'] = df['latent_calibration'].abs()
    log(f"  Mean |calibration|: {df['abs_calibration'].mean():.4f}")

    return df


def _step00_load_sem_calibration_OLD() -> pd.DataFrame:
    """
    OLD VERSION - Load and compute calibration from scratch.
    Kept for reference but not used.
    """
    log_section("STEP 00: Load SEM Latent Calibration Data (OLD)")

    ACCURACY_FILE = None  # Not defined
    CONFIDENCE_FILE = None  # Not defined
    ...
    df_merged['latent_calibration'] = df_merged['theta_confidence_z'] - df_merged['theta_accuracy_z']
    df_merged['abs_calibration'] = df_merged['latent_calibration'].abs()

    log(f"  Calibration range: [{df_merged['latent_calibration'].min():.3f}, {df_merged['latent_calibration'].max():.3f}]")
    log(f"  Mean calibration: {df_merged['latent_calibration'].mean():.4f}")
...
def step01_fit_lmm(df: pd.DataFrame) -> dict:
    ...
    formula = "latent_calibration ~ C(Domain) * TSVR_centered"
    ...
    formula_no_interaction = "latent_calibration ~ C(Domain) + TSVR_centered"
    try:
        if random_slope:
...
def step02_post_hoc_contrasts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    domain_stats = df.groupby('Domain')['latent_calibration'].agg(['mean', 'std', 'count']).reset_index()
...
def main():
    ...
    try:
        ...
        df_calibration = step00_load_sem_calibration()
