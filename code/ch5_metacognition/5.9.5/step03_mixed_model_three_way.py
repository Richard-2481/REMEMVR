...
"""
Step ID: step03
Step Name: Mixed Model Three-Way Interaction (measure x location x time)
RQ: results/ch6/6.9.5
Generated: 2026-01-19

PURPOSE:
Fit single mixed model with measure as a factor to test measure x location x
time three-way interaction. This is the PRIMARY hypothesis test for confidence-
accuracy dissociation in source-destination effects.

EXPECTED INPUTS:
  - results/ch5/5.5.1/data/step03_theta_scores.csv (accuracy, WIDE format)
  - results/ch6/6.8.1/data/step03_theta_confidence.csv (confidence, WIDE format)

EXPECTED OUTPUTS:
  - results/ch6/6.9.5/data/step03_mixed_model_results.csv
    Columns: term, beta, se, t_stat, df, p_uncorrected, p_bonferroni
    Rows: 8+ fixed effects (includes measure:location:TSVR_hours)
    Decision D068: BOTH p_uncorrected AND p_bonferroni present

  - results/ch6/6.9.5/data/step03_merged_data_long.csv
    Columns: UID, location, TSVR_hours, test, measure, theta, se
    Rows: 1600 (800 obs x 2 measures)
    Used by Steps 4, 6, 7, 8

  - results/ch6/6.9.5/data/step03_mixed_model_assumptions.txt
    Comprehensive assumption diagnostics from validate_lmm_assumptions_comprehensive

VALIDATION CRITERIA:
  - Model convergence
  - Three-way interaction term present
  - Both p_uncorrected and p_bonferroni columns (Decision D068)
  - Assumption diagnostics (normality, homoscedasticity, linearity, etc.)

g_code REASONING:
- Approach: WIDE->LONG reshape, merge accuracy+confidence, stack measures,
  fit theta ~ measure*location*TSVR_hours
- Why: Single model tests dissociation directly via 3-way interaction
- Data flow: 2 WIDE files -> 2 LONG (800 rows each) -> MERGE (800 rows) ->
  STACK measures (1600 rows) -> LMM -> Fixed effects table
- Expected performance: ~15 minutes (large model with 3-way interaction)

IMPLEMENTATION NOTES:
- Analysis tool: Direct statsmodels.formula.api.mixedlm (not fit_lmm_trajectory_tsvr wrapper)
- Validation tools: validate_lmm_convergence, validate_lmm_assumptions_comprehensive,
  validate_hypothesis_test_dual_pvalues
- Decision D068: Apply Bonferroni correction (alpha=0.05, n_tests=1 for primary hypothesis)
- CRITICAL: Normalize test column to 1/2/3/4 format before merging
- CRITICAL: Merge on (UID, test) instead of composite_ID
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 3: Mixed Model Three-Way Interaction")
        ...
        log(f"[RESHAPE] Accuracy: {len(df_acc_wide)} -> {len(df_acc_long)} rows")
        ...
        log(f"[RESHAPE] Confidence: {len(df_conf_wide)} -> {len(df_conf_long)} rows")
        ...
        log(f"[DEBUG] Accuracy test values: {sorted(df_acc_long['test'].unique())}")
        log(f"[DEBUG] Confidence test values: {sorted(df_conf_long['test'].unique())}")
        ...
        log(f"[MERGED] {len(df_merged)} rows (should be 800)")

        if len(df_merged) != 800:
            log(f"[ERROR] Expected 800 rows, got {len(df_merged)}")
            log(f"[DEBUG] Accuracy UIDs: {df_acc_long['UID'].nunique()}")
            log(f"[DEBUG] Confidence UIDs: {df_conf_long['UID'].nunique()}")
        ...
        log(f"[STACK] {len(df_merged)} -> {len(df_long)} rows (should be 1600)")
        ...
        log(f"[CONVERGED] Mixed model converged successfully")
        ...
        if three_way_terms:
            ...
            log(f"[WARN] Three-way interaction term not found")
        ...
        if result['d068_compliant']:
            log(f"[PASS] Decision D068 compliant: both p-values present")
        else:
        ...
        if assumption_result['valid']:
            log(f"[PASS] All assumption checks passed")
        else:
            log(f"[WARN] Some assumption checks failed: {assumption_result['message']}")
        ...
        log(f"[SAVE] {assumption_path.name}")
        ...
        log(f"[SAVE] {fe_path.name} ({len(df_fixed)} rows)")
        ...
        log(f"[SAVE] {data_path.name} ({len(df_long)} rows)")
