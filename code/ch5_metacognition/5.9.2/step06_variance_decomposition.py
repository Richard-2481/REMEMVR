...
"""
Step ID: step06
Step Name: Variance Decomposition
RQ: results/ch6/6.9.2
Generated: 2026-01-19

PURPOSE:
Decompose total ICC difference into components: scaling artifact, modeling artifact,
and genuine psychological variance. Components MUST sum to 100%. This quantifies
how much of the 824x ratio is artifact vs genuine individual differences.
...
EXPECTED OUTPUTS:
  - results/ch6/6.9.2/data/step06_variance_decomposition.csv
    Columns: ['component', 'variance_explained', 'percent_of_total', 'interpretation']
    Expected rows: 4 (3 components + total)
    Note: Total row MUST have percent_of_total = 100%

  - results/ch6/6.9.2/data/step06_decomposition_validation.csv
    Columns: ['metric', 'value', 'threshold', 'pass_fail']
    Expected rows: 3

  - results/ch6/6.9.2/data/step06_alternative_decomposition.csv
    Columns: ['method', 'D_total', 'scaling_pct', 'notes']
    Expected rows: 2

VALIDATION CRITERIA:
  - All percent_of_total >= 0
  - Total row: percent_of_total = 100% (within 0.01% tolerance)
  - Components sum validation: pass_fail = 'PASS'

g_code REASONING:
- Approach: Load all ICC estimates, define variance components with explicit
  formulas, compute percentages, validate sum to 100%
- Why this approach: Explicit variance decomposition ensures all sources accounted for
- Data flow: Load ICCs → Define components → Compute percentages → Validate sum
- Expected performance: ~10 minutes (pandas operations)

DECOMPOSITION FORMULAS:
  D_total = ICC_conf_single - ICC_acc (total difference to explain)
  D_scale = |ICC_conf_single - ICC_binary_conf| (scaling artifact)
  D_model = |ICC_conf_single - ICC_conf_ma| (modeling artifact)
  D_genuine = D_total - D_scale - D_model (residual)

  Scaling_pct = (D_scale / D_total) × 100%
  Modeling_pct = (D_model / D_total) × 100%
  Genuine_pct = (D_genuine / D_total) × 100%

  VALIDATION: Scaling_pct + Modeling_pct + Genuine_pct = 100%
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 06: Variance Decomposition")

        # =====================================================================
        # STEP 1: Load All ICC Estimates from Steps 1-5
        # =====================================================================

        log("\n[LOAD] Loading ICC estimates from prior steps...")
        ...
        # =====================================================================
        # STEP 2: Define Variance Components
        # =====================================================================
        ...
        log("\n[COMPUTE] Computing variance decomposition components...")
        ...
        # =====================================================================
        # STEP 4: Create Primary Decomposition Table
        # =====================================================================

        log("\n[CREATE] Creating variance decomposition table...")
        ...
        log(f"[SAVED] {output_decomp.name}")

        # =====================================================================
        # STEP 5: Create Validation Table
        # =====================================================================

        log("\n[VALIDATE] Creating validation checks...")
        ...
        # =====================================================================
        # STEP 6: Alternative Decomposition (if PowerLaw matched available)
        # =====================================================================

        log("\n[CREATE] Creating alternative decomposition...")
        ...
        log(f"[SAVED] {output_alt.name}")

        # =====================================================================
        # STEP 7: Interpretation
        # =====================================================================

        log("\n[INTERPRET] Variance decomposition interpretation:")
        ...
        log("\n[SUCCESS] Step 06 complete - Variance decomposition computed")
        sys.exit(0)
