...
if __name__ == "__main__":
    try:
        ...
        log(f"Domains present: {sorted(theta_data['domain'].unique())}")
        log(f"Tests per domain: {theta_data.groupby('domain').size().to_dict()}")
        ...
        for domain in ['What', 'Where', 'When']:
            ...
            if discrim_col in df_items.columns:
                ...
                if len(domain_items) > 0:
                    ...
                    log(f"Mean discrimination for {domain}: {mean_discrim[domain]:.4f} ({len(domain_items)} items)")
                else:
                    log(f"No items found for {domain} domain (domain excluded after purification)")
            else:
