#!/usr/bin/env python3
"""IRT Calibration Pass 1 (All TC_* Items): Calibrate 3-factor GRM on all TC_* confidence items (What/Where/When domains)."""
...
IRT_CONFIG = {
    ...
    'device': 'cpu',
    'seed': 42,
    'factors': ['What', 'Where', 'When']  # 3-factor structure
}
...
if __name__ == "__main__":
    try:
        log("Step 01: IRT Calibration Pass 1 (3-factor GRM on all TC_* items)")
        log(f"Model: 3-factor GRM, {IRT_CONFIG['n_cats']} categories")
        ...
        for domain in IRT_CONFIG['factors']:
            ...
            groups[domain] = items_in_domain
            log(f"{domain}: {len(items_in_domain)} items")
        ...
        log("Configuring 3-factor GRM model via configure_irt_model...")
