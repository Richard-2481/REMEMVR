#!/usr/bin/env python3
"""IRT Calibration Pass 2 (Purified Items): Calibrate 3-dimensional GRM on purified items only (Pass 2 of D039 - FINAL)."""
...
if __name__ == "__main__":
    try:
        ...
        log(f"Total items in original data: {len(df_wide.columns) - 1}")
        log(f"Purified items retained: {len(df_purified)}")
        # Filter Data to Purified Items Only
        ...
        log(f"Retained {n_items_retained} items in wide-format data")
        ...
        if missing_items:
            log(f"{len(missing_items)} purified items not found in data:")
        ...
        log("Converting filtered wide format to long format for IRT...")
        ...
        log("Deriving factor groups for purified items...")
        ...
        log("Running calibrate_irt (Pass 2 - purified items only)...")
        ...
        log(f"Items: {df_long['item_name'].nunique()} (purified)")
        ...
        log("Formatting and saving FINAL item parameters...")
        ...
        log("Item parameters (Pass 2 - FINAL):")
        ...
        log("Formatting and saving FINAL theta scores...")
        ...
        log("Additional parameter checks (D039 bounds):")
        ...
        log(f"Discrimination in [0.4, 10.0]: {a_valid} (actual: [{a_min:.3f}, {a_max:.3f}])")
        ...
        log(f"Difficulty in [-3.0, 3.0]: {b_valid} (actual: [{b_min:.3f}, {b_max:.3f}])")
        ...
        log("Step 03: IRT Calibration Pass 2 (FINAL) complete")
        log("")
