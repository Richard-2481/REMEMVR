#!/usr/bin/env python3
"""Compute Post-Hoc Contrasts (Decision D068): Compute pairwise domain contrasts with dual p-value reporting (Decision D068) and"""
...
if __name__ == "__main__":
    try:
        ...
        log("Step 06: Compute Post-Hoc Contrasts (Decision D068)")
        log("=" * 60)
        # Re-fit Best LMM Model (Log model)
        ...
        log("\nLoading LMM input data and re-fitting best model...")
        ...
        log(f"step04_lmm_input.csv ({len(df)} rows)")
        ...
        log("Re-fitting Log model (best by AIC)...")
        ...
        # Compute Pairwise Contrasts (Decision D068)
        #               dual p-value reporting (uncorrected + Bonferroni)

        log("\nRunning compute_contrasts_pairwise...")
        ...
        log(f"\nContrasts computed: {len(contrasts_df)} comparisons")
        # Save Contrasts Output
        ...
        log(f"\nSaved contrasts: {contrasts_output_path}")
        ...
        log("\nRunning compute_effect_sizes_cohens...")
        ...
        log(f"Effect sizes computed: {len(effect_sizes_df)} effects")
        # Save Effect Sizes Output
        ...
        log(f"\nSaved effect sizes: {effect_sizes_output_path}")
        ...
        if len(contrasts_df) != 3:
            ...
            log("  Exactly 3 pairwise contrasts computed")
        ...
        if missing_cols:
            ...
            log("  Both uncorrected AND Bonferroni p-values present")
        ...
        if not any("p_corrected mismatch" in e for e in validation_errors):
            log("  p_corrected = min(1.0, p_uncorrected * 3) verified")

        # Check 4: Effect sizes computed
        if len(effect_sizes_df) == 0:
            validation_errors.append("No effect sizes computed")
        else:
            log(f"  Effect sizes computed for {len(effect_sizes_df)} effects")
        ...
        if z_vals.isna().any() or not all(pd.notna(z_vals)):
            ...
            log("  z values finite (no NaN/Inf)")
        ...
        log("\n" + "=" * 60)
        log("Step 06 complete: Post-hoc contrasts computed")
        log("=" * 60)
        log("\nOutputs created:")
