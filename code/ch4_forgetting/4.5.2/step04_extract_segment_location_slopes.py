#!/usr/bin/env python3
"""step04_extract_segment_location_slopes: Extract 4 segment-location slopes from piecewise LocationType × Segment interaction LMM."""
...
if __name__ == "__main__":
    try:
        log("Step 4: Extract segment-location slopes")
        # Load Fitted LMM Model

        log("Loading fitted piecewise LMM model...")
        ...
        log(f"Piecewise LMM model from {model_path.name}")
        ...
        log(f"Model has {len(coef_names)} fixed effects coefficients")
        log(f"Coefficients: {coef_names}")
        ...
        log("Extracting variance-covariance matrix...")
        vcov_full = lmm_model.cov_params()
        log(f"Full variance-covariance matrix shape: {vcov_full.shape}")
        ...
        log(f"Fixed effects vcov matrix shape: {vcov_matrix.shape}")
        ...
        log("Identifying coefficient indices for slope calculations...")
        ...
        log(f"Days_within index: {idx_days_within}")
        log(f"Days_within:Segment[T.Late] index: {idx_days_segment_late}")
        log(f"Days_within:LocationType[T.Destination] index: {idx_days_location_dest}")
        log(f"Days_within:Segment[T.Late]:LocationType[T.Destination] index: {idx_days_segment_location}")
        # Compute 4 Segment-Location Slopes
        ...
        log("Computing segment-location slopes via linear combinations...")
        ...
        log(f"Source_Early slope: {slope_source_early:.6f}")
        log(f"Source_Late slope: {slope_source_late:.6f}")
        log(f"Destination_Early slope: {slope_destination_early:.6f}")
        log(f"Destination_Late slope: {slope_destination_late:.6f}")
        # Propagate Standard Errors via Delta Method
        # Delta method for linear combinations: SE²(aX + bY + cZ) =
        #   a²Var(X) + b²Var(Y) + c²Var(Z) + 2abCov(X,Y) + 2acCov(X,Z) + 2bcCov(Y,Z)

        log("Applying delta method for SE propagation...")
        ...
        log(f"Source_Early SE: {se_source_early:.6f}")
        log(f"Source_Late SE: {se_source_late:.6f}")
        log(f"Destination_Early SE: {se_destination_early:.6f}")
        log(f"Destination_Late SE: {se_destination_late:.6f}")
        ...
        log("Saving segment-location slopes...")
        ...
        log(f"{output_path.name} ({len(df_slopes)} rows, {len(df_slopes.columns)} columns)")
        # Run Validation Tool
        ...
        if not (0 <= p_min and p_max <= 1):
            log(f"ERROR: p-values outside [0, 1]: min={p_min:.6f}, max={p_max:.6f}")
            raise ValueError("p-values outside valid range")

        # Check for NaN values
        nan_count = df_slopes.isna().sum().sum()
        if nan_count > 0:
            log(f"ERROR: {nan_count} NaN values found in output")
