#!/usr/bin/env python3
"""Extract VR Data for Omnibus IRT Analysis: Extract VR test item responses from dfData.csv, dichotomize scores,"""
...
if __name__ == "__main__":
    try:
        ...
        log("Step 00: Extract VR Data for Omnibus IRT Analysis")
        log(f"RQ: 5.1.1 (General Type ROOT)")
        ...
        log(f"Filtered to {len(item_cols)} interactive paradigm items (IFR/ICR/IRE)")
        log(f"Excluded {len(all_item_cols) - len(item_cols)} non-interactive items (RFR/TCR)")
        # Create composite_ID
        ...
        # Create Q-Matrix (Omnibus "All" Factor)
        log("Creating Q-matrix with omnibus 'All' factor...")
        ...
        log(f"All {len(df_qmatrix)} items load on single 'All' factor (omnibus)")
        # Validate Outputs
        ...
        log("")
        log("=" * 80)
        log("Step 00 complete - General Type ROOT extraction")
        log("=" * 80)
        ...
        log("")
        log("This is the ROOT extraction for General type (5.1.X)")
