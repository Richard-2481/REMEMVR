#!/usr/bin/env python3
"""Prepare Functional Form Plot Data: Create dual-scale plot data (theta scale + probability scale) for visualizing"""
...
if __name__ == "__main__":
    try:
        log("Step 7: Prepare Functional Form Plot Data")
        # Load Input Data

        log("Loading observed theta scores...")
        ...
        log(f"{observed_path.name} ({len(lmm_input)} rows)")

        log("Loading best model identification...")
        ...
        log("Skipping pickle loading (patsy unpickling issue)")
        log("Refitting best model for predictions...")
        ...
        log(f"{best_model_name} model")
        # Compute Observed Means per Test Session
        ...
        # Generate Predictions for Best Model Only
        # Note: Only best model refitted (to avoid pickle unpickling)

        log(f"Generating predictions for {best_model_name} model...")
        ...
        log("Predictions generated for best model")
        # Combine Observed + Predictions (Theta Scale)

        log("Combining observed + predictions (theta scale)...")
        ...
        # Transform to Probability Scale (Decision D069)
        # Formula: p = 1 / (1 + exp(-1.7 * theta))

        log("Converting to probability scale (Decision D069)...")
        ...
        # Save Plot Data (Both Scales)
        # Output will be used by: plotting pipeline for visualization
        ...
        log(f"Saving theta-scale plot data to {theta_output_path.name}...")
        theta_plot_data.to_csv(theta_output_path, index=False, encoding='utf-8')
        log(f"{theta_output_path.name} ({len(theta_plot_data)} rows)")
        ...
        log(f"Saving probability-scale plot data to {prob_output_path.name}...")
        prob_plot_data[prob_cols].to_csv(prob_output_path, index=False, encoding='utf-8')
        log(f"{prob_output_path.name} ({len(prob_plot_data)} rows)")
        # Validate Plot Data
        ...
        if valid_ci_theta:
            log("Theta CI bounds valid (CI_upper > CI_lower)")
        else:
        ...
        if len(theta_plot_data) == expected_rows:
            log(f"Row count correct ({expected_rows} rows)")
        else:
            log(f"Row count differs from expected - got {len(theta_plot_data)}, expected ~{expected_rows}")
