...
# MAIN PROCESSING
def main():
    log("Step 01: Merge Data and Prepare LMM Input")
    log("")
    ...
    log(f"Theta wide: {len(theta_wide)} rows, {len(theta_wide.columns)} columns")

    tsvr_mapping = pd.read_csv(DATA_DIR / "step00_tsvr_mapping.csv", encoding='utf-8')
    log(f"TSVR mapping: {len(tsvr_mapping)} rows, {len(tsvr_mapping.columns)} columns")

    age_data = pd.read_csv(DATA_DIR / "step00_age_data.csv", encoding='utf-8')
    log(f"Age data: {len(age_data)} rows, {len(age_data.columns)} columns")
    log("")
    ...
    log(f"Theta + TSVR: {len(merged)} rows")

    # Check for merge failures
    nan_count = merged['UID'].isna().sum()
    if nan_count > 0:
        log(f"Merge introduced {nan_count} NaN values in UID column")
        return False
    log(f"Merge successful, no NaN values introduced")
    log("")
    ...
    log(f"Theta + TSVR + Age: {len(merged)} rows")

    # Check for merge failures
    nan_count = merged['Age'].isna().sum()
    if nan_count > 0:
        log(f"Merge introduced {nan_count} NaN values in Age column")
        return False
    log(f"Merge successful, no NaN values introduced")
    log(f"Columns after merge: {list(merged.columns)}")
    log("")
    ...
    log(f"Long format: {len(lmm_input)} rows")
    log(f"Expected: 1200 rows (400 x 3 congruence levels)")

    if len(lmm_input) != 1200:
        log(f"Row count mismatch: expected 1200, got {len(lmm_input)}")
        return False
    ...
    log(f"Age mean across 100 participants: {age_mean:.2f}")
    log(f"Age SD across 100 participants: {unique_ages.std():.2f}")
    ...
    log(f"Age_c mean across participants: {age_c_mean:.4f}")

    if abs(age_c_mean) > 0.1:
        log(f"Age_c mean not approximately 0: {age_c_mean:.4f}")
        return False
    log(f"Age_c mean within +/- 0.1 of 0: {age_c_mean:.4f}")
    log("")
    ...
    log(f"log_TSVR = log(TSVR_hours + 1)")
    log(f"log_TSVR range: [{lmm_input['log_TSVR'].min():.2f}, {lmm_input['log_TSVR'].max():.2f}]")
    ...
    if congruence_levels != expected_levels:
        ...
        return False
    log(f"Congruence levels correct: {sorted(congruence_levels)}")
    ...
    if len(incorrect_counts) > 0:
        log(f"Some UIDs don't have 12 observations:")
        log(f"  {incorrect_counts.to_dict()}")
        return False
    log(f"Each UID appears exactly 12 times (4 tests x 3 congruence)")
    log("")
    ...
    log("")
    log("Step 01 complete - LMM input prepared with Recip+Log transformations")
