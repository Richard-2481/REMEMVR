#!/usr/bin/env python3
"""Correlation Analysis with Steiger's Z-Test: Test if Purified CTT correlates more strongly with IRT theta than Full CTT does."""
...
if __name__ == "__main__":
    try:
        ...
        log(f"Full CTT: {len(ctt_full)} rows, {len(ctt_full.columns)} cols")
        log(f"Full CTT columns: {list(ctt_full.columns)}")
        log(f"Full CTT test column dtype: {ctt_full['test'].dtype}")

        # Load Purified CTT scores
        ctt_purified = pd.read_csv(RQ_DIR / "data/step03_ctt_purified_scores.csv")
        log(f"Purified CTT: {len(ctt_purified)} rows, {len(ctt_purified.columns)} cols")
        log(f"Purified CTT test column dtype: {ctt_purified['test'].dtype}")

        # Load IRT theta scores
        theta_long = pd.read_csv(RQ_DIR / "data/step00_theta_scores.csv")
        log(f"IRT theta (long format): {len(theta_long)} rows, {len(theta_long.columns)} cols")
        log(f"Theta domain names: {theta_long['domain_name'].unique()}")
        # Reshape Theta Scores from Long to Wide Format
        log("Pivoting theta scores to wide format...")

        theta_wide = pivot_theta_long_to_wide(theta_long)
        log(f"Theta wide: {len(theta_wide)} rows, columns: {list(theta_wide.columns)}")
        ...
        # Standardize Test Columns and Create Composite IDs
        log("Standardizing test columns and creating composite IDs...")
        ...
        log(f"Theta composite_ID sample: {theta_wide['composite_ID'].head(3).tolist()}")
        # Merge All Three Score Types on composite_ID
        log("Merging Full CTT, Purified CTT, and IRT theta scores...")
        ...
        log(f"After CTT merge: {len(merged)} rows")
        ...
        log(f"After theta merge: {len(merged)} rows")
        log(f"Final columns: {list(merged.columns)}")

        # Validation: Check for missing data
        missing_count = merged.isnull().sum().sum()
        if missing_count > 0:
            log(f"{missing_count} missing values detected in merged data")
            log(f"Missing by column:\n{merged.isnull().sum()}")
        # Compute Correlations and Run Steiger's Z-Test for Each Paradigm
        log("Running Steiger's z-test for each paradigm...")
        ...
        for paradigm in paradigms:
            ...
            n = len(data_subset)
            log(f"{paradigm}: N={n} observations after removing missing values")

            if n < 20:
                log(f"{paradigm}: Sample size ({n}) too small for Steiger's test (requires n>=20)")
                continue
            ...
            log(f"{paradigm}: r(IRT, Full CTT) = {r_full:.3f}")
            log(f"{paradigm}: r(IRT, Purified CTT) = {r_purified:.3f}")
            log(f"{paradigm}: r(Full CTT, Purified CTT) = {r_12:.3f}")
            ...
            log(f"{paradigm}: z = {z_stat:.3f}, p_uncorrected (one-tailed) = {p_uncorrected_onetailed:.4f}")
            log(f"{paradigm}: delta_r = {delta_r:.3f} (r_purified - r_full)")
        ...
        log("Applying Bonferroni correction for 3 comparisons...")
        ...
        log(f"Bonferroni correction: p_bonferroni = min(p_uncorrected × {n_comparisons}, 1.0)")

        # Log results
        for _, row in results_df.iterrows():
            log(f"{row['paradigm']}: p_uncorrected={row['p_uncorrected']:.4f}, p_bonferroni={row['p_bonferroni']:.4f}")
            if row['p_bonferroni'] < 0.05:
                log(f"{row['paradigm']}: SIGNIFICANT at alpha=0.05 (Bonferroni-corrected)")
            else:
                log(f"{row['paradigm']}: NOT significant at alpha=0.05")
        # Save Outputs
        ...
        log(f"{report_path}")
        # Validate Output with D068 Compliance Check
        log("Validating D068 compliance (dual p-value reporting)...")
        ...
        log(f"D068 compliance: BOTH p-value columns present")
        ...
        log(f"All correlations in valid range [-1, 1]")
        ...
        log(f"p_bonferroni capped at 1.0 (max observed: {max_p_bonf:.4f})")
        ...
        log(f"All {len(paradigms)} paradigms present in results")
