#!/usr/bin/env python3
"""Standardize Features: Standardize all 6 features to z-scores (mean=0, SD=1) for equal weighting"""
...
if __name__ == "__main__":
    try:
        ...
        log(f"Loading wide-format random effects...")
        ...
        # Compute Pre-Standardization Statistics

        log("Computing pre-standardization statistics...")

        pre_stats = df_wide.describe().loc[['mean', 'std']]
        log(f"Pre-standardization statistics:")
        ...
        log("Standardizing features to z-scores...")
        ...
        for col in FEATURE_COLS:
            ...
            log(f"Standardized {col} -> {z_col}")

        log(f"Standardization complete: {len(df_standardized.columns)} columns")
        # Compute Post-Standardization Statistics
        # Verify: mean~0, SD~1 for all z-score columns

        log("Computing post-standardization statistics...")
        ...
        for z_col in z_cols:
            ...
            log(f"{z_col}: mean={mean_val:.6f}, SD={std_val:.6f}")
        # Save Outputs
        # Output 1: Standardized features (100 rows × 7 columns)
        # Output 2: Standardization summary (pre/post statistics)

        log(f"Saving standardized features...")
        df_standardized.to_csv(OUTPUT_FILE, index=False, encoding='utf-8')
        log(f"{OUTPUT_FILE} ({len(df_standardized)} rows, {len(df_standardized.columns)} cols)")

        # Write summary report
        log(f"Saving standardization summary...")
