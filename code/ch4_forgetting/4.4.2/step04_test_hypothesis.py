...
PURPOSE:
Extract all 11 hypothesis tests from the fitted piecewise LMM and report with
dual p-value reporting (uncorrected + Bonferroni-corrected per Decision D068).
Primary hypothesis: Does the congruent slope differ between Early consolidation
and Late decay phases more than the common slope?
...
EXPECTED OUTPUTS:
  - results/step04_hypothesis_tests.csv
    Columns: ['Test_Name', 'Coefficient', 'SE', 'z_value', 'p_uncorrected',
              'p_bonferroni', 'Significant_Bonferroni']
    Expected rows: 11 (all fixed effects from 3-way interaction model)
    Description: 11 hypothesis tests with dual p-values

VALIDATION CRITERIA:
  - Test count: 11 rows (all LMM fixed effects)
  - Dual p-values present: p_uncorrected and p_bonferroni columns exist
  - p-value bounds: p_uncorrected in [0, 1]; p_bonferroni = p_uncorrected * 15
  - Bonferroni correction applied: p_bonferroni >= p_uncorrected for all
  - Significance labeling: Significant_Bonferroni = TRUE if p_bonferroni < 0.05
  - Primary hypothesis present: Days_within:Segment[Late]:Congruence[Congruent] term exists

g_code REASONING:
- Approach: Extract fixed effects from LMM + apply Bonferroni correction
- Why this approach: Decision D068 requires dual p-value reporting for transparency
- Data flow: LMM model -> extract_fixed_effects_from_lmm -> apply correction
- Expected performance: ~1 second (extraction only, no model fitting)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_lmm.extract_fixed_effects_from_lmm
- Validation: Inline validation (check dual p-values and correction)
- Bonferroni correction: n=15 tests (conservative family-wise error correction)
- Primary hypothesis: 3-way interaction term (congruent slope difference across segments)
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 4: Test Key Hypothesis - Congruent Consolidation Benefit")

        # =========================================================================
        # STEP 1: Load Fitted LMM Model
        # =========================================================================
        # Expected: MixedLMResults object with 11 fixed effects
        # Purpose: Extract all hypothesis tests
        ...
        # =========================================================================
        # Tool: tools.analysis_lmm.extract_fixed_effects_from_lmm
        # Returns: DataFrame with coefficient, SE, z-value, p-value

        log("[ANALYSIS] Extracting fixed effects from LMM...")
        ...
        log(f"[CORRECTION] Applying Bonferroni correction (n_tests = {N_TESTS})...")
        ...
        log(f"[CORRECTION] Applied Bonferroni correction (alpha = {ALPHA_BONFERRONI:.4f})")

        # =========================================================================
        # STEP 4: Identify Primary Hypothesis
        # =========================================================================
        ...
        log("[HYPOTHESIS] Identifying primary hypothesis test...")
        ...
        if len(primary_test) == 0:
            log("[WARNING] Primary hypothesis test not found (3-way interaction for Congruent)")
        else:
            ...
            log(f"[HYPOTHESIS] Primary test: {primary_name}")
        ...
        log(f"[PASS] p_uncorrected in valid range [0, 1]")
        ...
        log(f"[PASS] p_bonferroni in valid range [0, 1]")
        ...
        log(f"[PASS] Bonferroni correction applied correctly (p_bonf >= p_uncorr)")
        ...
        log(f"[PASS] Significance labeling correct")
        ...
        log(f"[RESULTS] Significant tests:")
        ...
        if n_sig_bonf > 0:
            log(f"[RESULTS] Significant tests (Bonferroni-corrected):")
        ...
        log(f"[SAVED] {output_path.name} ({len(df_tests)} rows)")
