#!/usr/bin/env python3
"""Prepare Piecewise LMM Input from RQ 5.1: Prepare piecewise LMM input by adding Segment and Days_within variables to RQ 5.1"""
...
if __name__ == "__main__":
    try:
        ...
        log(f"RQ 5.2.1 dependency exists: {RQ521_INPUT}")
        # Load Input Data

        log("Loading RQ 5.2.1 LMM input data...")
        df_rq51 = pd.read_csv(RQ521_INPUT, encoding='utf-8')
        log(f"{RQ521_INPUT.name} ({len(df_rq51)} rows, {len(df_rq51.columns)} cols)")
        log(f"Columns: {list(df_rq51.columns)}")
        ...
        log("Excluding When domain (floor effect per RQ 5.2.1)...")
        original_rows = len(df_rq51)
        original_domains = df_rq51["domain"].unique()
        log(f"Original domains: {list(original_domains)}")
        log(f"Original row count: {original_rows}")
        ...
        log(f"After filter - domains: {list(filtered_domains)}")
        log(f"After filter - row count: {filtered_rows}")
        log(f"Rows removed (When domain): {original_rows - filtered_rows}")
        log(f"When domain excluded successfully")
        ...
        log("Segment assignment by test:")
        ...
        log(f"Segment assignment complete: no NaN values")
        # Create Days_within Variable
        ...
        log(f"Row count preserved (after When filter): {len(df_piecewise)}")
        ...
        log("Segment assignment correct: test 1,2 -> Early; test 3,4 -> Late")
        ...
        log("No NaN values in Segment or Days_within")
        ...
        log(f"Days_within range valid: Early max={early_max:.2f}, Late max={late_max:.2f}")
        ...
        log(f"Expected domains present (When excluded): {expected_domains}")
        # Save Output
        # Save piecewise LMM input for Step 1 (model fitting)

        log("Saving piecewise LMM input...")
        ...
        log(f"{OUTPUT_FILE} ({len(df_piecewise)} rows, {len(output_columns)} cols)")
        log(f"Output columns: {output_columns}")
        # Final Summary

        log("")
        log("=" * 70)
        log("Step 00 complete: Piecewise LMM Input Prepared")
        log("=" * 70)
        log(f"  Input:  {RQ521_INPUT}")
        log(f"  Output: {OUTPUT_FILE}")
