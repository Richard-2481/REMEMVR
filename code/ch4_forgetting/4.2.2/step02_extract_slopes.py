#!/usr/bin/env python3
"""Extract Segment-by-Domain Slopes: Extract 4 segment-domain specific forgetting slopes via linear combinations"""
...
def compute_segment_domain_slopes(
    ...
    log(f"days_segment_late: {days_segment_late}")
    log(f"days_domain_where: {days_domain_where}")
    log(f"days_seg_dom_where: {days_seg_dom_where}")
...
if __name__ == "__main__":
    try:
        ...
        log("Step 02: Extract Segment-by-Domain Slopes")
        log("=" * 60)
        # Load Input Data (LMM model from step01)

        log("Loading piecewise LMM model from step01...")
        ...
        # Run Analysis Tool - Extract Fixed Effects

        log("\nExtracting fixed effects from LMM...")
        df_fixed_effects = extract_fixed_effects_from_lmm(lmm_model)
        log(f"Fixed effects extracted: {len(df_fixed_effects)} terms")
        ...
        # Compute Segment-Domain Slopes

        log("\nComputing segment-domain slopes via linear combinations...")
        df_slopes = compute_segment_domain_slopes(lmm_model)
        log(f"Slopes computed: {len(df_slopes)} segment-domain combinations")
        ...
        log(f"  Fixed effects table has {len(df_fixed_effects)} rows (>= 5 required)")
        ...
        if len(df_slopes) != 4:
            ...
            log(f"  Slope table has {len(df_slopes)} rows (4 required - When excluded)")
        ...
        if len(invalid_ci) > 0:
            ...
            log(f"  CI structure valid (CI_lower < slope < CI_upper) for all rows")
