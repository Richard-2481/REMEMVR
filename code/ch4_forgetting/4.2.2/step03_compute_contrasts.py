#!/usr/bin/env python3
"""Compute Planned Contrasts with Bonferroni Correction (Decision D068): Compute 3 planned contrasts with dual p-value reporting (D068). These contrasts"""
...
if __name__ == "__main__":
    try:
        log("Step 03: Compute Planned Contrasts (Decision D068)")
        log("=" * 60)
        # Load Input Data
        ...
        log(f"results/step02_segment_domain_slopes.csv ({len(df_slopes)} rows)")
        ...
        log(f"Slopes file has all required columns")
        ...
        log(f"All {expected_combinations} segment-domain combinations present (When excluded)")
        # Compute 3 Planned Contrasts (When excluded)

        log("\nComputing 3 planned contrasts (When domain excluded)...")
        log("=" * 60)
        ...
        log(f"Computed effect sizes for {len(df_effect_sizes)} terms")
        ...
        log(f"results/step03_planned_contrasts.csv ({len(df_contrasts)} rows)")

        # Save effect sizes
        effects_path = RQ_DIR / "results" / "step03_effect_sizes.csv"
        df_effect_sizes.to_csv(effects_path, index=False, encoding='utf-8')
        log(f"results/step03_effect_sizes.csv ({len(df_effect_sizes)} rows)")
        # Validation
        ...
        if len(df_contrasts) != 3:
            ...
            log("Contrast count correct: 3 rows (When excluded)")
        ...
        if missing_p_cols:
            ...
            log("Dual p-values present (D068 compliant)")
        ...
        if not np.isclose(df_contrasts['alpha_corrected'].iloc[0], expected_alpha, rtol=0.001):
            ...
            log(f"Bonferroni alpha correct: {expected_alpha:.4f}")
        ...
        if (df_contrasts['p_uncorrected'] < 0).any() or (df_contrasts['p_uncorrected'] > 1).any():
            ...
            log("p-values valid and correctly ordered")
        ...
        if len(df_effect_sizes) == 0:
            ...
            log(f"Effect sizes computed: {len(df_effect_sizes)} terms")
        ...
        for _, row in df_sorted.iterrows():
            sig_str = "" if row['sig_corrected'] else "[sig]" if row['sig_uncorrected'] else "[ns]"
