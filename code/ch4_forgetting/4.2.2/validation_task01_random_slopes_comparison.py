"""
validation TASK 1-2: Random Slopes Justification + Boundary Warning Investigation
...
logprint("=" * 70)
logprint("validation TASK 1-2: Random Slopes vs Intercepts-Only Comparison")
logprint("=" * 70)
logprint("")
...
logprint(f"{data.shape[0]} observations, {data.shape[1]} columns")
logprint(f"N participants: {data['UID'].nunique()}")
logprint(f"Domains: {sorted(data['domain'].unique())}")
logprint("")
...
logprint(f"Formula: {formula}")
logprint(f"Random effects: ~Days_within (intercepts + slopes)")
logprint("")
...
try:
    ...
    logprint("Model with random slopes converged")
    ...
    if slope_var < 0.02:
        logprint(f"Random slope variance very small ({slope_var:.6f})")
        logprint(f"       This explains boundary warning")
    
    slopes_success = True
    
except Exception as e:
    logprint(f"Random slopes model failed: {e}")
...
logprint(f"Formula: {formula}")
logprint(f"Random effects: ~1 (intercepts only)")
logprint("")
...
try:
    ...
    logprint("Intercepts-only model converged")
    ...
    logprint(f"Intercepts-only model failed: {e}")
...
if slopes_success and intercepts_success:
    ...
    logprint("=" * 70)
    logprint("")
    logprint("Boundary warning: 'MLE may be on boundary of parameter space'")
    logprint(f"Random slope variance = {slope_var:.6f} (very small)")
    logprint("")
    logprint("")
    ...
    if recommendation == "KEEP_SLOPES" or recommendation == "KEEP_SLOPES_CONSERVATIVE":
        logprint("Boundary warning is ACCEPTABLE:")
        ...
        logprint("Boundary warning suggests SIMPLER MODEL:")
    ...
    if recommendation == "SWITCH_TO_INTERCEPTS":
        ...
        logprint(f"Alternative model: {alt_model_path}")
        logprint("Model change recommended - requires user approval")

else:
    logprint("Model comparison failed")
