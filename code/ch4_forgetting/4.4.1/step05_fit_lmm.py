...
PURPOSE:
Fit 5 candidate LMMs with Congruence x Time interactions and select best by AIC.
Tests linear, quadratic, logarithmic, and combined time functions.
...
EXPECTED OUTPUTS:
  - results/step05_model_comparison.csv
    Columns: model_name, AIC, BIC, delta_AIC, AIC_weight
    Format: AIC comparison table
    Expected rows: 5
  ...
  - data/step05_lmm_fitted_model.pkl
    Format: Pickled best model for post-hoc contrasts

VALIDATION CRITERIA:
  - All 5 models converged
  - Best model has finite AIC
  - Fixed effects are finite (no NaN/Inf)
  - Random effect variance > 0

g_code REASONING:
- Approach: Use compare_lmm_models_by_aic with custom formulas for congruence analysis
- Why this approach: Need to test multiple time functions to find best fit
- Data flow: LMM input -> fit 5 models -> compare AIC -> select best -> save
- Expected performance: ~1-2 minutes (5 LMM fits)
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 05: LMM Model Fitting and Selection")
        ...
        for model_name, config in CANDIDATE_MODELS.items():
            ...
            if model_path.exists():
                ...
                try:
                    ...
                    log(f"    [WARN] Failed to load saved model: {e}")
            ...
            try:
                ...
                log(f"    [ERROR] {model_name} model failed: {str(e)}")
                fitted_models[model_name] = None
                aics[model_name] = np.inf

        # =========================================================================
        # STEP 3: Create AIC Comparison Table
        # =========================================================================
        log("\n[COMPARE] Creating AIC comparison table...")
        ...
        if best_model.converged:
            ...
            log("[WARN] Best model did NOT converge")

        # Check AIC is finite
        if np.isfinite(best_model.aic):
            log(f"[PASS] AIC is finite: {best_model.aic:.2f}")
        else:
        ...
        if fe_nan > 0 or fe_inf > 0:
            log(f"[WARN] Fixed effects have NaN ({fe_nan}) or Inf ({fe_inf})")
        else:
            log("[PASS] All fixed effects are finite")
        ...
        if re_var > 0:
            log(f"[PASS] Random effect variance: {re_var:.4f}")
        else:
            log(f"[WARN] Random effect variance <= 0: {re_var:.4f}")
        ...
        log("\n[SUCCESS] Step 05 complete (LMM Model Fitting)")
        sys.exit(0)
