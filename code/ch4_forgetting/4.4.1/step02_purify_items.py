...
PURPOSE:
Remove items with extreme difficulty or low discrimination per Decision D039 thresholds.
Implements 2-pass IRT methodology to ensure only psychometrically sound items
contribute to ability estimates.

EXPECTED INPUTS:
  - logs/step01_pass1_item_params.csv
    Columns: item_name, dimension, a, b
    Format: Pass 1 item parameters
    Expected rows: 72

EXPECTED OUTPUTS:
  - data/step02_purified_items.csv
    Columns: item_name, dimension, a, b, retention_reason
    Format: Items retained after D039 purification
    Expected rows: 50-65 items (30-50% removal expected)

  - data/step02_removed_items.csv
    Columns: item_name, dimension, a, b, removal_reason
    Format: Items excluded by D039 thresholds
    Expected rows: 10-25 items

VALIDATION CRITERIA:
  - All retained items have a >= 0.4
  - All retained items have |b| <= 3.0
  - Minimum 5 items per congruence category
  - Total >= 30 items retained
  - Retention rate 40-80%

g_code REASONING:
- Approach: Apply D039 thresholds (a >= 0.4, |b| <= 3.0)
- Why this approach: Low discrimination items add noise; extreme difficulty items are poorly estimated
- Data flow: Pass 1 params -> filter by thresholds -> purified + removed lists
...
if __name__ == "__main__":
    try:
        ...
        log(f"[LOADED] {INPUT_ITEM_PARAMS.name} ({len(df_pass1_params)} items)")
        ...
        # =========================================================================
        # STEP 2: Apply Purification Thresholds
        # =========================================================================
        log("\n[PURIFY] Applying D039 purification thresholds...")
        ...
        # =========================================================================
        # STEP 3: Report Purification Results
        # =========================================================================
        log("\n[RESULTS] Purification summary:")
        ...
        log(f"[SAVED] {OUTPUT_PURIFIED.name} ({len(df_purified)} items)")

        # Save removed items
        df_removed.to_csv(OUTPUT_REMOVED, index=False, encoding='utf-8')
        log(f"[SAVED] {OUTPUT_REMOVED.name} ({len(df_removed)} items)")

        # =========================================================================
        # STEP 5: Validation
        # =========================================================================
        log("\n[VALIDATE] Validating purification results...")
        ...
        for dim in ["common", "congruent", "incongruent"]:
            ...
            if n_dim < min_items_per_dim:
                log(f"[WARN] Only {n_dim} items retained for {dim} (minimum {min_items_per_dim})")
            else:
                log(f"[PASS] {dim}: {n_dim} items (>= {min_items_per_dim})")

        # Check minimum total items
        min_total = 30
        if n_purified < min_total:
            log(f"[WARN] Only {n_purified} items retained (recommended minimum {min_total})")
        else:
            log(f"[PASS] Total items: {n_purified} (>= {min_total})")

        # Check retention rate
        if retention_rate < 40:
            log(f"[WARN] Low retention rate: {retention_rate:.1f}% (expected 40-80%)")
        elif retention_rate > 80:
            log(f"[INFO] High retention rate: {retention_rate:.1f}% (may indicate lenient thresholds)")
        else:
            log(f"[PASS] Retention rate: {retention_rate:.1f}% (within expected 40-80%)")
        ...
        log("\n[SUCCESS] Step 02 complete (Item Purification)")
        sys.exit(0)
