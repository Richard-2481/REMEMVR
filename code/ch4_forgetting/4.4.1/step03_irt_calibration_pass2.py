#!/usr/bin/env python3
"""IRT Calibration Pass 2 (Purified Items): Calibrate final 3-dimensional GRM on purified items only. This is Pass 2, which"""
...
if __name__ == "__main__":
    try:
        ...
        log(f"{INPUT_PURIFIED.name} ({len(purified_items)} purified items)")
        # Filter to Purified Items Only
        log("\nFiltering to purified items only...")

        # Get columns to keep
        keep_columns = ["composite_ID"] + purified_items
        df_irt_filtered = df_irt_wide[keep_columns].copy()

        log(f"{len(purified_items)} item columns retained")
        ...
        log("\nRunning IRT calibration (Pass 2 - Final)...")
        ...
        if nan_counts.sum() > 0:
            log(f"NaN in theta scores: {nan_counts.to_dict()}")
        else:
            log("No NaN in theta scores")
        ...
        for col in se_cols:
            ...
            if se_mean > 0.5:
                log(f"High mean SE for {col}: {se_mean:.3f}")
            else:
                log(f"{col} mean: {se_mean:.3f}")
        ...
        if a_violations > 0:
            log(f"{a_violations} items have a < 0.4")
        if b_violations > 0:
            log(f"{b_violations} items have |b| > 3.0")
        if a_violations == 0 and b_violations == 0:
            log("All items within D039 thresholds")

        log("\nStep 03 complete (IRT Pass 2 - Final)")
        sys.exit(0)
