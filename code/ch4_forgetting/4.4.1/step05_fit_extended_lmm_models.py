"""
RQ 5.4.1 - Step 05: Fit Extended LMM Model Suite (Kitchen Sink)

PURPOSE:
Compare 70+ LMM trajectory models with Congruence × Time interaction using
comprehensive time transformations to identify best-fitting functional form.

INPUT:
- data/step04_lmm_input.csv (1200 rows × 9 columns: UID, composite_ID, test, congruence, theta, se, TSVR_hours, TSVR_sq, TSVR_log)
...
- Congruence × Time interaction (factor1_var='congruence')
...
CONTEXT:
- Original step05 (5 models): Log wins with 99.98% weight (overconfident)
- step05b (17 models): Recip+Log wins with 73.7% weight
- This step (70+ models): Full kitchen sink to verify Recip+Log robustness
...
if __name__ == "__main__":
    try:
        log("=" * 80)
        log("[START] Step 05: Kitchen Sink LMM Model Comparison (Congruence × Time)")
        log("=" * 80)
        ...
        results = compare_lmm_models_kitchen_sink(
            ...
            factor1_reference='common',  # Treatment coding: common (neutral) as reference
            factor2_var=None,
        ...
        best_weight = comparison.iloc[0]['aic_weight']
        if best_weight < 0.30:
            log("")
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("RQ 5.4.1 - Kitchen Sink Model Selection Summary\n")
            f.write("=" * 80 + "\n\n")
