#!/usr/bin/env python3
"""step00_get_data_from_rq521: Extract theta scores and TSVR from RQ 5.2.1 step04_lmm_input.csv (already in LONG"""
...
if __name__ == "__main__":
    try:
        log("Step 00: Get Data from RQ 5.2.1")
        # Load RQ 5.2.1 lmm_input (LONG format with domain-specific theta)
        log("Loading theta scores from RQ 5.2.1 (LONG format)...")
        ...
        log(f"RQ 5.2.1 lmm_input: {len(lmm_input)} rows (LONG format)")
        ...
        log(f"Domains in input: {lmm_input['domain'].unique().tolist()}")
        log(f"Rows per domain before filter: {lmm_input.groupby('domain').size().to_dict()}")
        # Filter out When domain (floor effect)
        log("Excluding When domain due to floor effect (RQ 5.2.1)...")

        # RQ 5.2.1 uses lowercase domain names
        theta_filtered = lmm_input[lmm_input['domain'].isin(['what', 'where'])].copy()

        log(f"{len(lmm_input)} rows -> {len(theta_filtered)} rows (When excluded)")
        log(f"Remaining domains: {theta_filtered['domain'].unique().tolist()}")
        ...
        if len(theta_long) != expected_rows:
            log(f"Expected {expected_rows} rows, got {len(theta_long)}")
        ...
        log("PASS - When excluded, 2 domains present")
        # Create TSVR mapping from lmm_input
        log("Creating TSVR mapping...")
        ...
        log(f"TSVR mapping: {len(tsvr_data)} rows")

        # Verify 400 rows (100 participants x 4 tests)
        if len(tsvr_data) != 400:
            log(f"Expected 400 TSVR rows, got {len(tsvr_data)}")
        ...
        log(f"Age data: {len(age_data)} unique participants")
        # Save outputs

        # Save LONG format theta scores (When excluded)
        log("Saving data/step00_theta_from_rq51.csv (LONG format, When excluded)...")
        theta_out_path = RQ_DIR / "data" / "step00_theta_from_rq51.csv"
        theta_long.to_csv(theta_out_path, index=False, encoding='utf-8')
        log(f"{theta_out_path.name} ({len(theta_long)} rows, {len(theta_long.columns)} cols)")

        # Save TSVR mapping
        log("Saving data/step00_tsvr_from_rq51.csv...")
        tsvr_out_path = RQ_DIR / "data" / "step00_tsvr_from_rq51.csv"
        tsvr_data.to_csv(tsvr_out_path, index=False, encoding='utf-8')
        log(f"{tsvr_out_path.name} ({len(tsvr_data)} rows, {len(tsvr_data.columns)} cols)")

        # Save age data (subset)
        log("Saving data/step00_age_from_dfdata.csv...")
        age_out_path = RQ_DIR / "data" / "step00_age_from_dfdata.csv"
        age_data.to_csv(age_out_path, index=False, encoding='utf-8')
        log(f"{age_out_path.name} ({len(age_data)} rows, {len(age_data.columns)} cols)")
        # Final validation
        log("Running final checks...")

        # Check theta LONG format (800 rows due to When exclusion)
        if len(theta_long) != 800:
            log(f"Expected 800 rows (400 x 2 domains, When excluded), got {len(theta_long)}")
        ...
        log("PASS - All files created successfully")
        log("Step 00 complete")
        log(f"Data sourced from RQ 5.2.1 (domain-specific theta)")
        log(f"LONG format theta ready: {len(theta_long)} rows (2 domains - When excluded)")
