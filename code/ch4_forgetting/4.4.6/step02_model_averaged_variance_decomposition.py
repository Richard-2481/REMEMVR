...
if __name__ == "__main__":
    try:
        log("=" * 80)
        log("Step 02: Model-Averaged Variance Decomposition")
        log("=" * 80)
        # Load LMM Input Data from RQ 5.4.1
        log("\nLoading LMM input data from RQ 5.4.1...")

        input_file = ROOT_RQ_DIR / "data" / "step04_lmm_input.csv"
        df_lmm = pd.read_csv(input_file, encoding='utf-8')

        log(f"{input_file.name}")
        ...
        log("\nComputing model-averaged variance decomposition...")
        ...
        # Save Model-Averaged Results
        log("\nSaving model-averaged results...")
        ...
        log(f"{variance_file.name} ({len(results['averaged_variance_components'])} rows)")

        # 3.2: ICCs
        icc_file = DATA_DIR / "step02_averaged_iccs.csv"
        results['averaged_ICCs'].to_csv(icc_file, index=False, encoding='utf-8')
        log(f"{icc_file.name} ({len(results['averaged_ICCs'])} rows)")
        ...
        log(f"{re_file.name} ({len(results['averaged_random_effects'])} rows)")
        ...
        log(f"{model_specific_file.name} ({len(model_specific_variance)} rows)")
        # Save Model Selection Metadata
        log("\nSaving model selection metadata...")
        ...
        log(f"{metadata_file.name}")
        # Validation
        log("\nValidating model-averaged results...")

        # 5.1: Check sufficient models
        n_models = results['summary_stats']['n_models_competitive']
        if n_models < 3:
            log(f"Only {n_models} competitive models - results may be unstable")
        else:
            log(f"{n_models} competitive models used")
        ...
        for col in icc_cols:
            if col in iccs.columns:
                ...
                if min_val < 0 or max_val > 1:
                    log(f"{col} out of bounds: [{min_val:.4f}, {max_val:.4f}]")
                else:
                    log(f"{col} bounded in [0, 1]: [{min_val:.4f}, {max_val:.4f}]")
        ...
        log(f"Random effects means (should be ≈ 0):")
        log(f"  Mean intercept: {mean_intercept:.6f}")
        log(f"  Mean slope: {mean_slope:.6f}")

        if abs(mean_intercept) < 0.01 and abs(mean_slope) < 0.01:
            log("Random effects centered at zero")
        else:
            log("Random effects not well-centered (may indicate numerical issues)")
        # Summary Statistics
        log("\nModel-averaged variance decomposition results:")
        log("-" * 80)
        ...
        log("\n" + "=" * 80)
        log("Step 02 complete - Model-averaged variance decomposition")
        log("=" * 80)
