#!/usr/bin/env python3
"""Extract Marginal Means at Day 3 Midpoint: Extract paradigm-specific marginal means (predicted theta) at Day 3 midpoint (72 hours)."""
...
if __name__ == "__main__":
    try:
        ...
        log("Step 01: Extract Marginal Means at Day 3 Midpoint")
        # Verify Step 0 Complete
        log("Verifying Step 0 completed...")
        ...
        log("Step 0 confirmation file exists")
        ...
        log(f"All parameters:\n{params}")

        # Get covariance matrix for SE calculation
        cov_params = lmm_model.cov_params()
        log(f"Covariance matrix shape: {cov_params.shape}")
        # Identify Coefficient Names - EXACT MATCHING
        ...
        log("Identifying coefficient names (exact matching)...")
        ...
        log(f"Intercept: {intercept_idx}")
        log(f"log_Days: {log_days_idx}")
        log(f"Cued_Recall effect: {cued_recall_idx}")
        log(f"Recognition effect: {recognition_idx}")
        log(f"Cued interaction: {cued_interaction_idx}")
        log(f"Recognition interaction: {recog_interaction_idx}")
        # Compute Marginal Means at Day 3
        ...
        log(f"Computing marginal means at log(Day 3) = {LOG_DAY_3:.4f}...")
        ...
        log(f"b_intercept = {b_intercept:.4f}")
        log(f"b_log_days = {b_log_days:.4f} (this is the Free_Recall slope)")
        log(f"b_cued = {b_cued:.4f}")
        log(f"b_recog = {b_recog:.4f}")
        log(f"b_cued_int = {b_cued_int:.4f}")
        log(f"b_recog_int = {b_recog_int:.4f}")
        ...
        log(f"Free_Recall marginal mean = {mm_free_recall:.4f}")
        log(f"Cued_Recall marginal mean = {mm_cued_recall:.4f}")
        log(f"Recognition marginal mean = {mm_recognition:.4f}")
        # Compute Standard Errors Using Delta Method
        # SE = sqrt(gradient' * Cov * gradient)
        # gradient = d(prediction)/d(coefficients)

        log("Computing standard errors...")
        ...
        log(f"Free_Recall SE = {se_free:.4f}")
        log(f"Cued_Recall SE = {se_cued:.4f}")
        log(f"Recognition SE = {se_recog:.4f}")
        ...
        log("Building marginal means DataFrame...")
        ...
        log(f"Marginal means DataFrame:\n{marginal_means.to_string()}")
        # Validate Output
        ...
        if not marginal_means['marginal_mean'].between(-3, 3).all():
            ...
            log(f"Actual range: [{marginal_means['marginal_mean'].min():.4f}, {marginal_means['marginal_mean'].max():.4f}]")

        # Check SE range
        if not marginal_means['SE'].between(0.01, 1.0).all():
            log(f"Some SEs outside expected range [0.01, 1.0]")
            log(f"Actual range: [{marginal_means['SE'].min():.4f}, {marginal_means['SE'].max():.4f}]")
        ...
        log(f"{output_path} ({len(marginal_means)} rows)")

        log("Step 01 complete - Marginal means extracted")
        sys.exit(0)
