#!/usr/bin/env python3
"""Compare ICC Across Paradigms and Prepare Barplot Data: Compare ICC_slope_conditional across three paradigms (Free Recall, Cued Recall,"""
...
if __name__ == "__main__":
    try:
        ...
        log("Computing 95% confidence intervals for each paradigm...")
        ...
        for idx, row in icc_slope.iterrows():
            ...
            log(f"Processing {paradigm} (ICC = {icc_value:.4f})...")
        ...
        # Rank Paradigms by ICC Magnitude
        # Rank 1 = highest ICC (most between-person variance)

        log("Ranking paradigms by ICC magnitude...")
        ...
        for idx, row in comparison_df.iterrows():
            log(f"Rank {row['rank']}: {row['paradigm']} (ICC = {row['icc_slope_conditional']:.4f})")
        # Prepare Barplot Data CSV
        # Subset columns for plotting pipeline visualization (Phase 2)

        log("Preparing barplot data CSV for visualization...")
        ...
        log(f"Barplot data ready ({len(barplot_data)} rows, {len(barplot_data.columns)} cols)")
        ...
        if validation_errors:
            ...
            log("  ICC values in [0, 1] range")
            log("  CI_lower < icc_value < CI_upper for all paradigms")
            log("  Rank values in [1, 2, 3]")
            log("  No NaN values")
            log("  Correct row counts (3 rows each)")
