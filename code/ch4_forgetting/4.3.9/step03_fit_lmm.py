...
def main():
    try:
        log("Step 03: Fit Cross-Classified LMM")
        # Load LMM Input Data
        ...
        log(f"Dropped {n_dropped} rows with missing Response ({n_after} rows remaining)")

        # Summary statistics
        log(f"Participants: {df['UID'].nunique()}")
        log(f"Items: {df['Item'].nunique()}")
        log(f"Observations: {len(df)}")
        log(f"Paradigms: {df['paradigm'].unique()}")
        log(f"Response mean: {df['Response'].mean():.3f}")
        log(f"Difficulty_c range: [{df['Difficulty_c'].min():.3f}, {df['Difficulty_c'].max():.3f}]")
        log(f"Difficulty_c mean: {df['Difficulty_c'].mean():.6f}")
        log(f"Time range: [{df['Time'].min():.1f}, {df['Time'].max():.1f}] hours")
        # Fit Cross-Classified LMM
        log("Fitting cross-classified LMM...")
        log("Formula: Response ~ Time * Difficulty_c * C(paradigm)")
        log("Random effects: ~Time (random intercept + slope)")
        log("Groups: UID (participant-level)")
        ...
        for i, strategy in enumerate(convergence_strategies, 1):
            ...
            try:
                ...
                # Check convergence
                if result.converged:
                    log(f"Model converged with strategy {i}")
                    break
                else:
                    log(f"Model did not converge with strategy {i}")
                    if i < len(convergence_strategies):
                        log(f"Trying next strategy...")
                        continue
                    else:
                        log(f"All strategies exhausted, model did not converge")
                        raise ValueError("Model failed to converge with all strategies")

            except Exception as e:
                log(f"Strategy {i} failed: {str(e)}")
                if i < len(convergence_strategies):
                    log(f"Trying next strategy...")
                    continue
                else:
        ...
        # Extract Fixed Effects with Dual P-Values (D068)
        ...
        log(f"Bonferroni correction: n_tests = {n_tests}, alpha_bonf = {alpha_bonf:.4f}")
        # Extract Random Effects
        log("Extracting random effects...")
        ...
        log(f"Random effects variance components:")
        ...
        validation_result = validate_lmm_convergence(lmm_result=result)
        validation_result = validate_lmm_convergence(lmm_result=result)
        log(f"Convergence result: {validation_result}")

        if not validation_result.get('all_valid', False):
            log(f"Convergence validation issues: {validation_result}")
        ...
        log(f"Assumptions result: {assumptions_result}")

        if not assumptions_result.get('all_valid', False):
            log(f"Assumptions validation issues: {assumptions_result}")
        ...
        log(f"Hypothesis test result: {hypothesis_result}")

        if not hypothesis_result.get('valid', False):
            log(f"Hypothesis test validation failed: {hypothesis_result}")
