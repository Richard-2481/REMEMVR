"""
RQ 5.5.1 - Step 05: Fit Extended LMM Model Suite (Kitchen Sink)

PURPOSE:
Compare 70+ LMM trajectory models with LocationType × Time interaction using
comprehensive time transformations to identify best-fitting functional form.

INPUT:
- data/step04_lmm_input.csv (800 rows × 11 columns: UID, test, TSVR_hours, Days, LocationType, theta, etc.)
...
- LocationType × Time interaction (factor1_var='LocationType')
...
if __name__ == "__main__":
    try:
        ...
        required_cols = ['UID', 'theta', 'TSVR_hours', 'LocationType']
        ...
        log("  Factor 1: LocationType (source, destination)")
        log("  Interaction: LocationType × time_transform")
        ...
        results = compare_lmm_models_kitchen_sink(
            ...
            # LocationType × Time interaction
            factor1_var='LocationType',
            factor1_reference='source',  # Treatment coding: source as reference
            factor2_var=None,
        ...
        for i, row in comparison.head(10).iterrows():
            log(f"  {row['rank']:2d}. {row['model_name']:25s} "
                ...
                f"weight={row['aic_weight']*100:5.2f}%")
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("RQ 5.5.1 - Kitchen Sink Model Selection Summary\n")
            f.write("=" * 80 + "\n\n")
