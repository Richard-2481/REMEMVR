...
def main():
    logger.info("[START] Step 05: Prepare Age Tertile Plot Data")

    # -------------------------------------------------------------------------
    # 1. Load LMM input data
    # -------------------------------------------------------------------------
    logger.info("[LOAD] Loading LMM input data...")

    lmm_input = pd.read_csv(DATA_DIR / "step01_lmm_input.csv")
    logger.info(f"[LOADED] {len(lmm_input)} observations")

    # -------------------------------------------------------------------------
    # 2. Compute age tertile cutoffs
    # -------------------------------------------------------------------------
    logger.info("[TERTILE] Computing age tertile cutoffs...")
    ...
    logger.info(f"[CUTOFF] 33rd percentile: {p33:.1f} years")
    logger.info(f"[CUTOFF] 67th percentile: {p67:.1f} years")

    # -------------------------------------------------------------------------
    # 3. Assign age tertiles
    # -------------------------------------------------------------------------
    logger.info("[ASSIGN] Assigning observations to age tertiles...")
    ...
    logger.info("[COUNTS] Participants per tertile:")
    ...
    # -------------------------------------------------------------------------
    logger.info("[AGGREGATE] Computing means and CIs by group...")
    ...
    logger.info(f"[RESULT] {len(plot_df)} groups created")
    ...
    if len(plot_df) == expected_rows:
        logger.info(f"[PASS] Row count = {expected_rows}")
    else:
        logger.info(f"[FAIL] Expected {expected_rows} rows, found {len(plot_df)}")
        all_pass = False

    # Check 2: All tertiles present
    tertiles_present = set(plot_df['age_tertile'].unique())
    if tertiles_present == {'Young', 'Middle', 'Older'}:
        logger.info("[PASS] All 3 age tertiles present")
    else:
        logger.info(f"[FAIL] Missing tertiles: expected Young/Middle/Older, found {tertiles_present}")
        all_pass = False

    # Check 3: All locations present
    locations_present = set(plot_df['location'].unique())
    if locations_present == {'Source', 'Destination'}:
        logger.info("[PASS] Both locations present")
    else:
        logger.info(f"[FAIL] Missing locations: expected Source/Destination, found {locations_present}")
        all_pass = False

    # Check 4: All tests present
    tests_present = set(plot_df['test'].unique())
    if len(tests_present) == 4:
        logger.info("[PASS] All 4 tests present")
    else:
        logger.info(f"[FAIL] Missing tests, found: {tests_present}")
        all_pass = False

    # Check 5: No NaN values
    nan_count = plot_df.isna().sum().sum()
    if nan_count == 0:
        logger.info("[PASS] No NaN values")
    else:
        logger.info(f"[FAIL] Found {nan_count} NaN values")
        all_pass = False
    ...
    min_n = plot_df['n'].min()
    if min_n >= 5:
        logger.info(f"[PASS] Minimum group size = {min_n} (>= 5)")
    else:
        logger.info(f"[WARNING] Small group size: min n = {min_n}")
    ...
    # 6. Save output
    # -------------------------------------------------------------------------
    logger.info("[SAVE] Saving plot data...")

    output_path = DATA_DIR / "step05_age_tertile_plot_data.csv"
    plot_df.to_csv(output_path, index=False)
    logger.info(f"[SAVED] {output_path.name} ({len(plot_df)} rows)")

    # -------------------------------------------------------------------------
    # 7. Summary statistics
    # -------------------------------------------------------------------------
    logger.info("[SUMMARY] Theta means by age tertile and location:")
    ...
    logger.info("[SUCCESS] Step 05 complete - Plot data prepared")
