#!/usr/bin/env python3
"""Characterize Clusters: Characterize clusters by computing summary statistics (mean, SD) in raw scale"""
...
if __name__ == "__main__":
    try:
        ...
        log("Loading raw-scale random effects from Step 0...")
        random_effects = pd.read_csv(RQ_DIR / "data" / "step00_random_effects_from_rq514.csv")
        log(f"random_effects ({len(random_effects)} rows)")
        ...
        log(f"cluster_assignments ({len(cluster_assignments)} rows)")
        # Merge Data

        log("Merging random effects with cluster assignments on UID...")
        merged = pd.merge(random_effects, cluster_assignments, on='UID')
        log(f"{len(merged)} participants with cluster + raw intercept/slope")
        # Compute Cluster Summary Statistics

        log("Computing cluster summary statistics...")
        ...
        log("Assigning interpretive labels based on profile patterns...")
        ...
        log(f"Overall intercept median: {overall_intercept_median:.4f}")
        log(f"Overall slope median: {overall_slope_median:.4f}")
        ...
        log("Saving cluster characterization...")
        summary_df.to_csv(RQ_DIR / "data" / "step06_cluster_characterization.csv", index=False, encoding='utf-8')
        log(f"step06_cluster_characterization.csv ({len(summary_df)} rows)")

        log("Saving cluster labels...")
        ...
        with open(RQ_DIR / "data" / "step06_cluster_labels.txt", 'w', encoding='utf-8') as f:
            f.write(labels_text)
        log(f"step06_cluster_labels.txt")
        # Run Validation Tool

        log("Running validate_cluster_summary_stats...")

        # Validate that sum of N = 100
        total_n = summary_df['N'].sum()
        if total_n != 100:
            raise ValueError(f"Sum of N across clusters ({total_n}) != 100")
        log(f"Sum of N = {total_n} (expected 100): PASS")
