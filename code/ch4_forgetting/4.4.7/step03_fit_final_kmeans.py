...
if __name__ == "__main__":
    try:
        ...
        # Load optimal K
        log(f"Reading optimal K from {INPUT_K_FILE}")
        ...
        log(f"Fitting K-means with K={optimal_k}, n_init=50, random_state=42")
        ...
        log(f"Inertia = {kmeans.inertia_:.4f}")
        log(f"Cluster sizes:")
        ...
        # Save assignments
        log(f"Writing cluster assignments to {OUTPUT_ASSIGNMENTS_FILE}")
        df_assignments.to_csv(OUTPUT_ASSIGNMENTS_FILE, index=False, encoding='utf-8')
        log(f"{OUTPUT_ASSIGNMENTS_FILE}")

        # Save cluster centers
        log(f"Writing cluster centers to {OUTPUT_CENTERS_FILE}")
        ...
        log("Validating cluster assignments")
        ...
        if not validation_result['valid']:
            log(f"Validation issue: {validation_result['message']}")
            log("Proceeding despite validation warning (K=6 selected by BIC)")
        else:
            log(f"[VALIDATION PASS] {validation_result['message']}")

        log(f"Step 03 complete - {optimal_k} clusters assigned")
        sys.exit(0)
