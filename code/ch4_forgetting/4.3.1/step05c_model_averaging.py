"""
RQ 5.3.1 - Step 05c: Model Averaging for Robust Predictions
...
INPUT:
- data/step04_lmm_input.csv (training data with TSVR_hours × paradigm)
- data/model_comparison.csv (65 models, Akaike weights)

OUTPUT:
- data/step05c_averaged_predictions.csv (model-averaged trajectory per paradigm)
...
if __name__ == "__main__":
    try:
        log("=" * 80)
        log("[START] Step 05c: Model Averaging (Paradigm × Time)")
        log("=" * 80)
        ...
        log(f"  Paradigms: {sorted(lmm_input['paradigm'].unique())}")
        ...
        # Create prediction grid (per paradigm)
        ...
        log(f"  Grid points: {len(pred_grid)} ({len(tsvr_grid)} per paradigm)")

        # Compute model-averaged predictions
        log("\n[AVERAGING] Computing model-averaged predictions...")
        log("  NOTE: Paradigm × Time interaction - averaging across functional forms")
        ...
        sqrt_models = [m for m in models_used if 'SquareRoot' in m or 'CubeRoot' in m]

        log(f"  Power-law family: {len(power_models)} models")
        log(f"  Logarithmic family: {len(log_models)} models")
        log(f"  Root family: {len(sqrt_models)} models")
        ...
        with open(summary_path, 'w', encoding='utf-8') as f:
            f.write("=" * 80 + "\n")
            f.write("RQ 5.3.1 - Model Averaging Summary (Paradigm × Time)\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write(f"  Root family: {len(sqrt_models)} models (diminishing returns)\n\n")

            f.write(f"Effective Functional Form:\n")
            f.write(f"  Dominant family: Power-law/Log hybrid\n")
            f.write(f"  Interpretation: Proportional decay with paradigm-specific rates\n")
            f.write(f"  Paradigm interaction: Time transformation × Paradigm effects\n\n")
        ...
        # Report per-paradigm predictions
        log("\n[SUMMARY] Averaged predictions per paradigm:")
        for paradigm in paradigms:
            paradigm_preds = output[output['paradigm'] == paradigm]
            log(f"  {paradigm:15s}: θ ∈ [{paradigm_preds['theta_averaged'].min():.3f}, "
                f"{paradigm_preds['theta_averaged'].max():.3f}], "
                f"SE ∈ [{paradigm_preds['prediction_se'].min():.4f}, "
                f"{paradigm_preds['prediction_se'].max():.4f}]")
