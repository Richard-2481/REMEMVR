#!/usr/bin/env python3
"""IRT Calibration Pass 2 (Purified Items): Final IRT calibration on purified items (Decision D039 - 2-pass purification)."""
...
if __name__ == "__main__":
    try:
        log("Step 03: IRT Calibration Pass 2 (Purified Items)")
        ...
        log(f"step00_irt_input.csv ({len(df_irt_wide)} rows, {len(df_irt_wide.columns)} cols)")
        ...
        log(f"step02_purified_items.csv ({len(df_purified)} rows)")
        log(f"Purified items: {len(df_purified)} items retained")
        ...
        log("Filtering to purified items only...")
        ...
        log(f"IRT input: {len(df_irt_filtered.columns)-1} item columns retained")

        # Filter Q-matrix to purified items
        df_qmatrix_filtered = df_qmatrix[df_qmatrix['item_name'].isin(purified_item_names)].copy()
        log(f"Q-matrix: {len(df_qmatrix_filtered)} items retained")
        ...
        log(f"Long format: {len(df_long)} rows (400 obs x {len(item_cols)} items)")
        log(f"Unique UIDs: {df_long['UID'].nunique()}")
        log(f"Test sessions: {sorted(df_long['test'].unique())}")
        # Build Groups Dict from Filtered Q-Matrix

        log("Building groups dictionary from Q-matrix...")
        ...
        for factor, items in groups.items():
            log(f"Factor '{factor}': {len(items)} items")
        ...
        log(f"Groups patterns: {groups_patterns}")
        ...
        log(f"IRT config: n_factors=3, correlated_factors=True, device=cpu")
        # Run IRT Calibration (Pass 2)

        log("Running calibrate_irt (Pass 2 - Purified Items)...")
        ...
        # Transform Outputs to Required Format
        ...
        log("Transforming outputs to required format...")
        ...
        log(f"Theta scores: {len(df_theta_final)} rows")
        log(f"Unique composite_IDs: {df_theta_final['composite_ID'].nunique()}")
        log(f"Domain names: {df_theta_final['domain_name'].unique().tolist()}")
        ...
        log(f"Item parameters: {len(df_items_final)} rows")
        ...
        log(f"{theta_output_path} ({len(df_theta_final)} rows, {len(df_theta_final.columns)} cols)")
        ...
        log(f"{items_output_path} ({len(df_items_final)} rows, {len(df_items_final.columns)} cols)")
        # Run Validation
        ...
        if all_passed:
            log("Step 03 complete - All validation checks passed")
            sys.exit(0)
        else:
            log("Step 03 complete with validation warnings")
