...
"""
Step ID: step03
Step Name: Define Nested Models
RQ: results/ch7/7.8.3
Generated: 2026-01-09

PURPOSE:
Specify 4 nested regression models with increasing complexity. Models range from
minimal (Age+RAVLT) to full (8 predictors). Ensures predictors exist in analysis
dataset and models are properly nested.
...
EXPECTED OUTPUTS:
  - results/ch7/7.8.3/data/step03_nested_models.csv
    Columns: ['model_name', 'num_predictors', 'predictor_list', 'rationale']
    Expected rows: 4
    Format: Model specifications

  - results/ch7/7.8.3/data/step03_model_specs.txt
    Format: Human-readable model specifications

VALIDATION CRITERIA:
  - All predictors exist in analysis dataset
  - Models are properly nested (Minimal ⊂ Core ⊂ Extended ⊂ Full)
  - No duplicate predictors within models

g_code REASONING:
- Approach: Explicit predictor lists for each nested model
- Why this approach: Clear model hierarchy for parsimony testing
- Data flow: Define specifications -> validate predictors -> save
- Expected performance: ~seconds

IMPLEMENTATION NOTES:
- Analysis tool: Custom specification (stdlib operation)
- Validation tool: tools.validation.validate_dataframe_structure
- Parameters: 4 nested models (2, 3, 5, 8 predictors)
- NOTE: Full model has 8 predictors (not 9) - NART not available
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 03: Define Nested Models")
        ...
        log(f"[LOADED] Analysis dataset: {len(df_analysis)} rows, {len(df_analysis.columns)} columns")
        log(f"[INFO] Available columns: {df_analysis.columns.tolist()}")

        # =========================================================================
        # STEP 2: Validate Predictors Exist
        # =========================================================================
        log("[VALIDATION] Checking all predictors exist in dataset...")
        ...
        for model in NESTED_MODELS:
            ...
            log(f"[CHECK] Model '{model_name}': {len(predictor_list)} predictors")

            # Check each predictor exists
            missing_predictors = [p for p in predictor_list if p not in available_predictors]

            if missing_predictors:
                log(f"[ERROR] Model '{model_name}' references missing predictors: {missing_predictors}")
                all_valid = False
            else:
                log(f"[PASS] All predictors available for '{model_name}'")

        if not all_valid:
            raise ValueError("One or more models reference missing predictors")

        # =========================================================================
        # STEP 3: Validate Nesting Structure
        # =========================================================================
        log("[VALIDATION] Checking models are properly nested...")
        ...
        for i in range(len(NESTED_MODELS) - 1):
            ...
            if not current_predictors.issubset(next_predictors):
                ...
                log(f"[ERROR] Nesting violation: '{current_model['model_name']}' has predictors not in '{next_model['model_name']}': {extra_predictors}")
                all_valid = False
            else:
                log(f"[PASS] '{current_model['model_name']}' properly nested in '{next_model['model_name']}'")

        if not all_valid:
            raise ValueError("Models are not properly nested")

        # =========================================================================
        # STEP 4: Check for Duplicate Predictors
        # =========================================================================
        log("[VALIDATION] Checking for duplicate predictors within models...")
        ...
        for model in NESTED_MODELS:
            ...
            if len(predictor_list) != len(set(predictor_list)):
                ...
                log(f"[ERROR] Model '{model_name}' has duplicate predictors: {duplicates}")
                all_valid = False
            else:
                log(f"[PASS] No duplicate predictors in '{model_name}'")

        if not all_valid:
            raise ValueError("One or more models have duplicate predictors")

        # =========================================================================
        # STEP 5: Save Model Specifications (CSV)
        # =========================================================================
        log("[SAVE] Saving model specifications...")
        ...
        log(f"[SAVED] Model specifications: {models_file} ({len(df_models)} models)")

        # =========================================================================
        # STEP 6: Save Detailed Specifications (TXT)
        # =========================================================================
        specs_file = OUTPUT_DIR / "step03_model_specs.txt"
        log(f"[SAVE] Saving detailed specifications to {specs_file}")
        ...
        log(f"[SAVED] Detailed specifications: {specs_file}")
        ...
        if validation_result.get('valid', False):
            log(f"[PASS] Output structure validated")
        else:
            log(f"[ERROR] Output structure validation failed: {validation_result.get('message', '')}")
            raise ValueError("Output validation failed")

        # Print summary
        log("[SUMMARY] Model specifications:")
        for model in NESTED_MODELS:
