...
PURPOSE:
Perform sensitivity analyses to assess robustness of optimal model. Tests:
(1) outlier exclusion, (2) LOO-CV vs 5-fold CV, (3) alternative stratification,
(4) bootstrap seed stability. Ensures results are not artifacts of analytical choices.
...
EXPECTED INPUTS:
  ...
  - results/ch7/7.8.3/data/step07_outlier_analysis.csv (outlier identification)

EXPECTED OUTPUTS:
  - results/ch7/7.8.3/data/step08_outlier_sensitivity.csv (with/without outliers)
  - results/ch7/7.8.3/data/step08_loo_cv_results.csv (LOO-CV vs 5-fold)
  - results/ch7/7.8.3/data/step08_alternative_cv.csv (age vs theta stratification)
  - results/ch7/7.8.3/data/step08_bootstrap_stability.csv (seed 42 vs 43)

VALIDATION CRITERIA:
  - Coefficient stability across outlier exclusion
  - LOO-CV and 5-fold CV agree within 0.05 R²
  - Stratification method doesn't substantially change results
  - Bootstrap CIs stable across seeds (width ratio < 2.0)

g_code REASONING:
- Approach: Systematic perturbation of analytical choices
- Why this approach: Quantifies robustness, identifies fragile findings
- Data flow: Refit model under alternative conditions -> compare to main results
- Expected performance: ~minutes (LOO-CV is N=100 iterations)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_regression.fit_multiple_regression (repeated under conditions)
- Validation tool: tools.validation.validate_bootstrap_stability
- Parameters: Cook's D threshold=0.04, alternative seeds=[42, 43], theta tertiles for alt stratification
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        ...
        # =========================================================================
        # STEP 2: Sensitivity Test 1 - Outlier Exclusion
        # =========================================================================
        ...
        log(f"[INFO] Identified {n_outliers} outliers (Cook's D > {COOKS_D_THRESHOLD})")
        ...
        log(f"[RESULTS] Full data: R²={r2_full:.4f}, N={len(df_analysis)}")
        log(f"[RESULTS] No outliers: R²={r2_no_outliers:.4f}, N={len(X_no_outliers)}")
        log(f"[RESULTS] Coefficient stability (r): {coef_correlation:.4f}")
        log(f"[RESULTS] Significance changes: {significance_changes}")
        ...
        # =========================================================================
        # STEP 3: Sensitivity Test 2 - LOO-CV vs 5-Fold CV
        # =========================================================================
        log("\n[SENSITIVITY] Test 2: LOO-CV comparison...")
        ...
        log(f"[RESULTS] LOO-CV R²: {loo_cv_r2:.4f}")
        log(f"[RESULTS] 5-Fold CV R²: {fold5_cv_r2:.4f}")
        log(f"[RESULTS] Difference: {difference:.4f}")
        ...
        # =========================================================================
        # STEP 4: Sensitivity Test 3 - Alternative Stratification
        # =========================================================================
        log("\n[SENSITIVITY] Test 3: Alternative stratification (theta tertiles)...")
        ...
        log(f"[RESULTS] Age stratification: CV-R²={age_cv_r2_mean:.4f}")
        log(f"[RESULTS] Theta stratification: CV-R²={theta_cv_r2_mean:.4f} ± {theta_cv_r2_sd:.4f}")
        ...
        # =========================================================================
        # STEP 5: Sensitivity Test 4 - Bootstrap Seed Stability
        # =========================================================================
        ...
        log("[RESULTS] Bootstrap stability:")
        ...
        log(f"[SAVED] {bootstrap_file}")

        # =========================================================================
        # STEP 7: Validation Checks
        # =========================================================================
        log("\n[VALIDATION] Assessing sensitivity results...")

        # Coefficient stability
        if coef_correlation > STABILITY_THRESHOLD:
            log(f"[PASS] Coefficients stable after outlier exclusion (r={coef_correlation:.4f})")
        else:
            log(f"[WARNING] Coefficients unstable after outlier exclusion (r={coef_correlation:.4f})")

        # LOO-CV agreement
        if difference < 0.05:
            log(f"[PASS] LOO-CV agrees with 5-fold CV (diff={difference:.4f})")
        else:
            log(f"[WARNING] LOO-CV differs from 5-fold CV (diff={difference:.4f})")

        # Bootstrap stability
        max_ratio = df_bootstrap_stab['stability_ratio'].max()
        if max_ratio < MAX_CI_WIDTH_RATIO:
            log(f"[PASS] Bootstrap CIs stable across seeds (max ratio={max_ratio:.4f})")
        else:
            log(f"[WARNING] Bootstrap CI instability detected (max ratio={max_ratio:.4f})")
