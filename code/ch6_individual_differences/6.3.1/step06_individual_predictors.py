...
PURPOSE:
Examine individual cognitive test predictors with corrected significance tests
including dual p-values (Decision D068) and comprehensive diagnostics.
...
EXPECTED OUTPUTS:
  - results/ch7/7.3.1/data/step06_individual_predictors.csv
    ...
    Format: Individual predictor analysis with dual p-values
    Expected rows: 6
  - results/ch7/7.3.1/data/step06_assumption_diagnostics.txt
    Format: Regression assumption test results
    Expected size: 1000-2000 bytes

VALIDATION CRITERIA:
  - Model convergence successful
  - VIF values reasonable (most < 5)
  - All 6 predictors present with dual p-values

g_code REASONING:
- Approach: Multiple regression with individual predictors and Bonferroni correction
- Why this approach: Dual p-value reporting (Decision D068) with strict alpha for cognitive tests
- Data flow: Analysis dataset → regression model → individual coefficients with corrected p-values
- Expected performance: ~seconds (standard multiple regression)
...
- Parameters: Bonferroni alpha = 0.000597 (0.00179/3 cognitive tests)
...
if __name__ == "__main__":
    try:
        log("[START] Step 06: Individual Predictors Analysis")

        # =========================================================================
        # STEP 1: Load Analysis Dataset
        # =========================================================================
        # Expected: Complete analysis dataset with confidence theta and predictors
        # Purpose: Extract individual predictor effects on confidence

        log("[LOAD] Loading analysis dataset...")
        df_analysis = pd.read_csv(RQ_DIR / "data" / "step04_analysis_dataset.csv")
        log(f"[LOADED] Analysis dataset ({len(df_analysis)} rows, {len(df_analysis.columns)} cols)")
        ...
        log(f"[INFO] Sample size: {len(df_analysis)} participants")
        log(f"[INFO] Predictors: {required_cols[1:]}")  # Exclude outcome variable
        ...
        log(f"[PREP] Outcome variable: confidence_theta (mean={y.mean():.3f}, sd={y.std():.3f})")
        log(f"[PREP] Predictor matrix: {X.shape}")
        
        # Check for missing values
        missing_outcome = y.isna().sum()
        missing_predictors = X.isna().sum().sum()
        if missing_outcome > 0 or missing_predictors > 0:
            log(f"[WARNING] Missing values detected: outcome={missing_outcome}, predictors={missing_predictors}")
            # Drop missing cases
            ...
            log(f"[PREP] Complete cases: {len(complete_cases)} participants")

        # =========================================================================
        # STEP 3: Run Multiple Regression Analysis
        # =========================================================================
        # Use statsmodels directly due to function signature issues

        log("[ANALYSIS] Running multiple regression analysis...")
        ...
        log("[DONE] Multiple regression complete")
        ...
        log(f"[RESULTS] Model R² = {r2:.4f} (adjusted = {adj_r2:.4f})")
        log(f"[RESULTS] F({X.shape[1]}, {len(X)-X.shape[1]-1}) = {f_stat:.3f}, p = {f_pval:.6f}")

        # =========================================================================
        # STEP 4: Compute VIF and Semi-partial Correlations
        # =========================================================================
        # Calculate VIF for multicollinearity assessment
        log("[DIAGNOSTICS] Computing VIF values...")
        ...
        for i in range(1, X_with_const.shape[1]):  # Skip constant
            ...
            log(f"[VIF] {X.columns[i-1]}: {vif:.3f}")
        
        # Semi-partial correlations (sr²)
        log("[DIAGNOSTICS] Computing semi-partial correlations...")
        sr2_values = []
        ...
        # =========================================================================
        # STEP 5: Apply Multiple Comparisons Corrections (Decision D068)
        # =========================================================================
        # Dual p-values: uncorrected + Bonferroni
        # Bonferroni alpha = 0.000597 for cognitive tests

        log("[CORRECTIONS] Applying multiple comparisons corrections...")
        ...
        log(f"[CORRECTIONS] Bonferroni alpha for cognitive tests: {alpha_bonferroni:.6f}")
        log(f"[CORRECTIONS] FDR correction applied to all {len(p_uncorrected)} predictors")

        # =========================================================================
        # STEP 6: Create Individual Predictors Results DataFrame
        # =========================================================================
        
        log("[COMPILE] Creating individual predictors results...")
        ...
        for i, predictor in enumerate(X.columns):
            ...
            log(f"[SIGNIFICANCE] {predictor}: β={result['beta']:.4f}, "
        ...
        # =========================================================================
        # STEP 7: Save Individual Predictors Results
        # =========================================================================
        
        log("[SAVE] Saving individual predictors results...")
        output_path = RQ_DIR / "data" / "step06_individual_predictors.csv"
        individual_df.to_csv(output_path, index=False, encoding='utf-8')
        log(f"[SAVED] {output_path} ({len(individual_df)} rows, {len(individual_df.columns)} cols)")

        # =========================================================================
        # STEP 8: Save Assumption Diagnostics
        # =========================================================================
        
        log("[DIAGNOSTICS] Saving assumption diagnostics...")
...
"""
        ...
        log(f"[SAVED] {diag_path} ({len(diagnostics_text)} bytes)")
        ...
        log(f"[VALIDATION] Predictors with p_bonferroni < 0.05: {n_significant_bonferroni}/6")
        log(f"[VALIDATION] Cognitive tests with p < {alpha_bonferroni:.6f}: {n_significant_cognitive}/5")
        log(f"[VALIDATION] Maximum VIF: {max_vif:.3f}")
        log(f"[VALIDATION] Individual predictors analyzed: {len(individual_df)} coefficients")
