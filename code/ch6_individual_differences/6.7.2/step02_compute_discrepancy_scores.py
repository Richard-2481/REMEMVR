...
"""
Step ID: step02
Step Name: compute_discrepancy_scores
RQ: results/ch7/7.7.2
Generated: 2026-01-09

PURPOSE:
Calculate discrepancy scores (REMEMVR_z - RAVLT_z) and examine distribution.
Positive discrepancy = VR-favored (better on REMEMVR than RAVLT).
Negative discrepancy = RAVLT-favored (better on RAVLT than REMEMVR).

EXPECTED INPUTS:
  - results/ch7/7.7.2/data/step01_standardized_scores.csv
    Columns: ['UID', 'REMEMVR_z', 'RAVLT_z']
    Description: Standardized z-scores from step 1
    Expected rows: ~100

EXPECTED OUTPUTS:
  - results/ch7/7.7.2/data/step02_discrepancy_scores.csv
    Columns: ['UID', 'REMEMVR_z', 'RAVLT_z', 'Discrepancy']
    Description: Discrepancy scores for all participants
    Expected rows: ~100
  - results/ch7/7.7.2/data/step02_discrepancy_descriptives.csv
    Columns: ['statistic', 'value', 'ci_lower', 'ci_upper']
    Description: Distribution statistics with bootstrap CIs
    Expected rows: ~10 (mean, SD, median, IQR, min, max, skewness, kurtosis, normality test)

VALIDATION CRITERIA:
  - Discrepancy scores in reasonable range [-4, 4]
  - No missing values in discrepancy calculation
  - Discrepancy = REMEMVR_z - RAVLT_z verified

g_code REASONING:
- Approach: Compute discrepancy as REMEMVR_z - RAVLT_z, describe distribution, bootstrap CIs
- Why this approach: Discrepancy score quantifies individual divergence between measures
- Data flow: Load z-scores -> Calculate discrepancy -> Describe distribution -> Bootstrap CIs
- Expected performance: ~5 seconds (bootstrap 1000 iterations)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_extensions.compute_discrepancy_scores + scipy.stats + numpy bootstrap
- Validation tool: tools.validation.validate_numeric_range
- Parameters: bootstrap_iterations=1000, random_seed=42
- Discrepancy interpretation: Positive = VR-favored, Negative = RAVLT-favored, Near-zero = Concordant
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 02: Compute Discrepancy Scores")

        # =========================================================================
        # STEP 1: Load Standardized Scores
        # =========================================================================
        log("[LOAD] Loading standardized z-scores from step 1...")

        input_file = RQ_DIR / "data" / "step01_standardized_scores.csv"
        df = pd.read_csv(input_file)

        log(f"[LOADED] {len(df)} participants with standardized scores")
        ...
        log("[CALCULATE] Computing discrepancy scores (REMEMVR_z - RAVLT_z)...")
        ...
        log(f"[CALCULATED] Discrepancy scores for {len(df)} participants")

        # Verify calculations
        mean_discrepancy = df['Discrepancy'].mean()
        sd_discrepancy = df['Discrepancy'].std()
        log(f"[INFO] Discrepancy (Total): M={mean_discrepancy:.4f}, SD={sd_discrepancy:.4f}")

        mean_discrepancy_pr = df['Discrepancy_PctRet'].mean()
        sd_discrepancy_pr = df['Discrepancy_PctRet'].std()
        log(f"[INFO] Discrepancy (Pct Ret): M={mean_discrepancy_pr:.4f}, SD={sd_discrepancy_pr:.4f}")

        # =========================================================================
        # STEP 3: Describe Distribution (both metrics)
        # =========================================================================
        log("[DESCRIBE] Computing distribution statistics...")
        ...
        for metric_label, disc_col in discrepancy_metrics.items():
            ...
            log(f"[INFO] VR-favored ({disc_col} > 0): {vr_favored} ({vr_favored/len(df)*100:.1f}%)")
            log(f"[INFO] RAVLT-favored ({disc_col} < 0): {ravlt_favored} ({ravlt_favored/len(df)*100:.1f}%)")
            log(f"[INFO] Concordant ({disc_col} = 0): {concordant}")
            log(f"[INFO] Shapiro-Wilk normality test: W={shapiro_stat:.4f}, p={shapiro_p:.4f}")
            ...
            log(f"[BOOTSTRAP] {metric_label} Mean 95% CI: [{[r for r in all_ci_results if r['metric']==metric_label and r['statistic']=='mean'][0]['ci_lower']:.4f}, {[r for r in all_ci_results if r['metric']==metric_label and r['statistic']=='mean'][0]['ci_upper']:.4f}]")
        ...
        # STEP 5: Save Outputs
        # =========================================================================
        log("[SAVE] Saving discrepancy scores and descriptives...")
        ...
        log(f"[SAVED] {output_scores.name} ({len(df)} rows)")
        ...
        # =========================================================================
        # STEP 6: Validate Discrepancy Scores
        # =========================================================================
        ...
        for disc_col, z_col, label in [
            ...
            if validation_result.get('valid', False):
                log(f"[PASS] {label} discrepancy scores within reasonable range [-4, 4]")
            else:
                log(f"[WARNING] {label} validation warnings: {validation_result}")

            # Check for missing values
            n_missing = df[disc_col].isna().sum()
            if n_missing > 0:
                log(f"[ERROR] {n_missing} missing {label} discrepancy values")
                raise ValueError(f"Missing {label} discrepancy values detected")
            else:
                log(f"[PASS] No missing {label} discrepancy values")
            ...
            if not np.allclose(df[disc_col], calculated_discrepancy):
                log(f"[ERROR] {label} discrepancy calculation mismatch")
                raise ValueError(f"{disc_col} != REMEMVR_z - {z_col}")
            else:
                log(f"[PASS] {label} discrepancy calculation verified (REMEMVR_z - {z_col})")
