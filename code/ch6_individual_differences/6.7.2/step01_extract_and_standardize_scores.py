...
"""
Step ID: step01
Step Name: extract_and_standardize_scores
RQ: results/ch7/7.7.2
Generated: 2026-01-09

PURPOSE:
Load theta scores from Ch5 5.1.1 and RAVLT Total scores from dfnonvr.csv,
standardize both to z-scores for commensurable discrepancy calculation.
Bootstrap 95% CIs for standardization parameters (mean, SD).

EXPECTED INPUTS:
  - results/ch7/7.7.2/data/step00_dependency_validation.txt
    Description: Validation results with file paths
  ...
  - data/dfnonvr.csv
    ...
    Description: RAVLT trial scores for calculating total
    Expected rows: ~100

EXPECTED OUTPUTS:
  - results/ch7/7.7.2/data/step01_standardized_scores.csv
    Columns: ['UID', 'theta_raw', 'RAVLT_raw', 'REMEMVR_z', 'RAVLT_z']
    Description: Raw and standardized scores for both measures
    Expected rows: ~100
  - results/ch7/7.7.2/data/step01_standardization_stats.csv
    Columns: ['measure', 'mean', 'sd', 'mean_ci_lower', 'mean_ci_upper', 'sd_ci_lower', 'sd_ci_upper']
    Description: Bootstrap CIs for standardization parameters
    Expected rows: 2 (REMEMVR, RAVLT)
...
g_code REASONING:
- Approach: Load theta scores (aggregate across sessions), calculate RAVLT Total (sum trials 1-5), standardize both using sample statistics
- Why this approach: Z-scores make REMEMVR and RAVLT commensurable despite different scales
- Data flow: Load theta -> Aggregate to UID level -> Load RAVLT -> Calculate total -> Merge -> Standardize -> Bootstrap CIs
- Expected performance: ~10 seconds (bootstrap 1000 iterations)

IMPLEMENTATION NOTES:
- Analysis tool: scipy.stats.zscore + numpy bootstrap
- Validation tool: tools.validation.validate_standardization
- Parameters: bootstrap_iterations=1000, random_seed=42, confidence_level=0.95
- CRITICAL: RAVLT Total = sum of trials 1-5 ONLY (exclude distraction trial)
- CRITICAL: Theta scores aggregated by UID (mean across sessions)
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 01: Extract and Standardize Scores")

        # =========================================================================
        # STEP 1: Load Validation Results to Get File Paths
        # =========================================================================
        log("[LOAD] Reading dependency validation results...")
        ...
        log(f"[INFO] Theta file path: {theta_path.relative_to(PROJECT_ROOT)}")

        # =========================================================================
        # STEP 2: Load Ch5 Theta Scores and Aggregate by UID
        # =========================================================================
        # Purpose: Get one REMEMVR score per participant (mean across sessions)
        ...
        log(f"[INFO] Using theta column: '{theta_col}'")
        log(f"[LOADED] Theta scores: {len(theta_df)} rows")
        ...
        log(f"[AGGREGATED] {len(theta_agg)} unique participants (mean theta across sessions)")

        # =========================================================================
        # STEP 3: Load dfnonvr.csv and Calculate RAVLT Total
        # =========================================================================
        # Purpose: Calculate RAVLT Total from trials 1-5 (exclude distraction)
        # CRITICAL: Trials 1-5 only (from gcode_lessons.md Bug #6)

        log("[LOAD] Loading dfnonvr.csv for RAVLT data...")
        dfnonvr_path = PROJECT_ROOT / "data" / "dfnonvr.csv"
        dfnonvr = pd.read_csv(dfnonvr_path)

        log(f"[LOADED] dfnonvr.csv: {len(dfnonvr)} participants")
        ...
        log(f"[CALCULATED] RAVLT Total from trials 1-5 (post-ceiling-fix)")
        log(f"[INFO] RAVLT Total range: {dfnonvr['RAVLT_Total'].min():.1f} to {dfnonvr['RAVLT_Total'].max():.1f}")
        ...
        # =====================================================================
        log("[CALCULATE] Computing RAVLT Percent Retention...")
        ...
        log(f"[CALCULATED] RAVLT Percent Retention = DR / max(trials 1-5) * 100")
        log(f"[INFO] RAVLT Pct Ret range: {dfnonvr['RAVLT_Pct_Ret'].min():.1f}% to {dfnonvr['RAVLT_Pct_Ret'].max():.1f}%")
        log(f"[INFO] RAVLT Pct Ret M={dfnonvr['RAVLT_Pct_Ret'].mean():.1f}%, SD={dfnonvr['RAVLT_Pct_Ret'].std():.1f}%")

        # =========================================================================
        # STEP 4: Merge REMEMVR and RAVLT Data
        # =========================================================================
        log("[MERGE] Merging REMEMVR theta scores with RAVLT scores...")
        ...
        log(f"[MERGED] {len(merged)} participants with both REMEMVR and RAVLT scores")
        ...
        if theta_only:
            log(f"[WARNING] {len(theta_only)} participants in theta data but not in RAVLT data")
        if ravlt_only:
            log(f"[WARNING] {len(ravlt_only)} participants in RAVLT data but not in theta data")

        # =========================================================================
        # STEP 5: Standardize Both Measures to Z-Scores
        # =========================================================================
        # Purpose: Make REMEMVR and RAVLT commensurable for discrepancy calculation
        # Method: Sample-based z-scores (M=0, SD=1)

        log("[STANDARDIZE] Converting raw scores to z-scores...")
        ...
        log(f"[INFO] REMEMVR_z: M={merged['REMEMVR_z'].mean():.4f}, SD={merged['REMEMVR_z'].std():.4f}")
        log(f"[INFO] RAVLT_z: M={merged['RAVLT_z'].mean():.4f}, SD={merged['RAVLT_z'].std():.4f}")
        log(f"[INFO] RAVLT_Pct_Ret_z: M={merged['RAVLT_Pct_Ret_z'].mean():.4f}, SD={merged['RAVLT_Pct_Ret_z'].std():.4f}")

        # =========================================================================
        # STEP 6: Bootstrap 95% CIs for Standardization Parameters
        # =========================================================================
        # Purpose: Quantify uncertainty in mean and SD estimates
        # Method: Participant-level resampling (1000 iterations, seed=42)

        log("[BOOTSTRAP] Computing 95% CIs for standardization parameters...")
        ...
        log(f"[BOOTSTRAP] REMEMVR mean 95% CI: [{rememvr_mean_ci[0]:.4f}, {rememvr_mean_ci[1]:.4f}]")
        log(f"[BOOTSTRAP] REMEMVR SD 95% CI: [{rememvr_sd_ci[0]:.4f}, {rememvr_sd_ci[1]:.4f}]")
        log(f"[BOOTSTRAP] RAVLT mean 95% CI: [{ravlt_mean_ci[0]:.2f}, {ravlt_mean_ci[1]:.2f}]")
        log(f"[BOOTSTRAP] RAVLT SD 95% CI: [{ravlt_sd_ci[0]:.2f}, {ravlt_sd_ci[1]:.2f}]")
        log(f"[BOOTSTRAP] RAVLT Pct Ret mean 95% CI: [{pctret_mean_ci[0]:.2f}, {pctret_mean_ci[1]:.2f}]")
        log(f"[BOOTSTRAP] RAVLT Pct Ret SD 95% CI: [{pctret_sd_ci[0]:.2f}, {pctret_sd_ci[1]:.2f}]")
        ...
        log(f"[SAVED] {output_scores.name} ({len(final_df)} rows)")

        output_stats = RQ_DIR / "data" / "step01_standardization_stats.csv"
        stats_df.to_csv(output_stats, index=False, encoding='utf-8')
        log(f"[SAVED] {output_stats.name} ({len(stats_df)} rows)")

        # =========================================================================
        # STEP 8: Validate Standardization
        # =========================================================================
        ...
        if validation_result.get('valid', False):
            log("[PASS] Standardization validation successful")
        else:
            log(f"[WARNING] Standardization validation warnings: {validation_result}")
        ...
        if rememvr_outliers > 0:
            log(f"[WARNING] {rememvr_outliers} REMEMVR extreme outliers (|z| > 4)")
        if ravlt_outliers > 0:
            log(f"[WARNING] {ravlt_outliers} RAVLT extreme outliers (|z| > 4)")
        if pctret_outliers > 0:
            log(f"[WARNING] {pctret_outliers} RAVLT Pct Ret extreme outliers (|z| > 4)")
