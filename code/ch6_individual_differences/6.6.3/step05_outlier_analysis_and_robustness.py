...
"""
Step ID: step05
Step Name: outlier_analysis_and_robustness
RQ: results/ch7/7.6.3
Generated: 2026-01-09

PURPOSE:
Identify outliers in domain-specific slopes using z-score method and assess impact
on ICC estimates. Tests robustness of ICC conclusions to extreme values.
...
EXPECTED OUTPUTS:
  - results/ch7/7.6.3/data/step05_outlier_analysis.csv
    Columns: ['domain', 'UID', 'z_score', 'outlier_type']
    Format: Variable rows (only outliers listed)
  - results/ch7/7.6.3/data/step05_robustness_checks.csv
...
g_code REASONING:
- Approach: Z-score outlier detection (|z| > 3.29), recompute ICC without outliers
- Why this approach: Standard threshold (p < 0.001 two-tailed), sensitivity analysis
- Data flow: Slopes -> Z-scores -> Outlier flags -> ICC with/without outliers
- Expected performance: <1 second
- Threshold: 3.29 (p < 0.001, conservative)

IMPLEMENTATION NOTES:
- Analysis tool: scipy.stats.zscore
- Validation tool: validate_numeric_range from tools.validation
- Outlier threshold: |z| > 3.29 (p < 0.001 two-tailed)
- Also test Shapiro-Wilk normality (helps interpret outlier context)
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 05: Outlier Analysis and Robustness")
        ...
        for domain in ['what', 'where']:
            ...
            if outlier_pct > 10:
                log(f"[WARN] {domain.upper()}: {outlier_pct:.1f}% outliers (>10% threshold)")
            else:
                log(f"[PASS] {domain.upper()}: {outlier_pct:.1f}% outliers (<10% threshold)")
        ...
        # =========================================================================
        # STEP 4: Robustness Analysis (ICC with/without Outliers)
        # =========================================================================
        # Recompute ICC excluding outliers to assess impact

        log(f"[ROBUSTNESS] Recomputing ICC without outliers...")
        ...
        if len(outlier_records) > 0:
            ...
            log(f"[CREATED] Outlier analysis table: {len(df_outliers)} outliers")
        else:
            # No outliers - create empty DataFrame with correct columns
            df_outliers = pd.DataFrame(columns=['domain', 'UID', 'z_score', 'outlier_type'])
            log(f"[CREATED] Outlier analysis table: 0 outliers (none detected)")

        df_robust = pd.DataFrame(robustness_results)
        log(f"[CREATED] Robustness checks table: {len(df_robust)} rows")

        # =========================================================================
        # STEP 6: Validate Results
        # =========================================================================

        log(f"[VALIDATE] Checking robustness results...")
        ...
        if not validation_delta.get('valid', False):
            log(f"[WARN] Some delta_icc values outside [-0.2, 0.2]: {validation_delta}")
            log(f"[INFO] Large ICC changes may indicate influential outliers")
        else:
            log(f"[PASS] All delta_icc values in reasonable range [-0.2, 0.2]")

        # Check all domains present
        if len(df_robust) != 2:
            log(f"[ERROR] Expected 2 domains in robustness checks, got {len(df_robust)}")
            sys.exit(1)
        else:
            log(f"[PASS] Both domains represented in robustness checks")

        # Validate outlier z-scores (if any outliers)
        if len(df_outliers) > 0:
            for _, row in df_outliers.iterrows():
                if abs(row['z_score']) <= Z_THRESHOLD:
                    log(f"[ERROR] Outlier {row['UID']} has |z| = {abs(row['z_score']):.2f} <= {Z_THRESHOLD}")
                    sys.exit(1)

            log(f"[PASS] All flagged outliers have |z| > {Z_THRESHOLD}")

        # =========================================================================
        # STEP 7: Save Results
        # =========================================================================

        log(f"[SAVE] Writing outlier analysis...")
        df_outliers.to_csv(OUTPUT_OUTLIERS, index=False, encoding='utf-8')
        log(f"[SAVED] {OUTPUT_OUTLIERS} ({len(df_outliers)} outliers)")

        log(f"[SAVE] Writing robustness checks...")
        df_robust.to_csv(OUTPUT_ROBUST, index=False, encoding='utf-8')
        log(f"[SAVED] {OUTPUT_ROBUST} ({len(df_robust)} rows)")

        log(f"[SUCCESS] Step 05 complete")
        log(f"[SUMMARY] Outliers detected: {len(df_outliers)}, max |delta_icc|: {df_robust['delta_icc'].abs().max():.4f}")
        log(f"[NEXT] Proceed to step06 (split-half reliability)")
