...
"""
Step ID: step03
Step Name: correlational_analysis
RQ: results/ch7/7.5.3
Generated: 2026-01-06

PURPOSE:
Primary correlational analysis and t-test with Decision D068 dual p-value reporting

EXPECTED INPUTS:
  - results/ch7/7.5.3/data/step02_analysis_dataset.csv
    Columns: ['theta_all', 'rehearsal_frequency', 'mnemonic_use']
    Format: CSV with analysis-ready data
    Expected rows: ~100

EXPECTED OUTPUTS:
  - results/ch7/7.5.3/data/step03_correlation_results.csv
    Columns: ['analysis', 'r', 'p_uncorrected', 'p_bonferroni', 'CI_lower', 'CI_upper', 'n']
    Format: CSV with Pearson correlation between rehearsal frequency and theta_all
    Expected rows: 1

  - results/ch7/7.5.3/data/step03_group_comparison.csv
    Columns: ['analysis', 'mean_group1', 'mean_group2', 't_statistic', 'df', 'p_uncorrected', 'p_bonferroni', 'cohens_d', 'CI_lower', 'CI_upper', 'n1', 'n2']
    Format: CSV with t-test comparing mnemonic users vs non-users
    Expected rows: 1

VALIDATION CRITERIA:
  - dual_p_values: Both uncorrected and Bonferroni-corrected p-values present
  - required_cols: All expected statistical columns present

g_code REASONING:
- Approach: Pearson correlation + independent t-test with bootstrap CIs and D068 dual p-values
- Why this approach: Standard approach for continuous-ordinal correlation and binary group comparison
- Data flow: Analysis dataset → Correlation analysis → Group comparison → Dual p-value correction
- Expected performance: ~30 seconds (correlation and t-test calculations)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_stats.t_test_d068 for group comparison with dual p-values
- Validation tool: validate_correlation_test_d068 for checking dual p-value format
- Parameters: Bonferroni correction with n_comparisons=2 (correlation + t-test)
- Bootstrap CIs: 1000 replications with seed=42 for reproducible confidence intervals
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 03: Correlational analysis")

        # =========================================================================
        # STEP 1: Load Analysis Dataset
        # =========================================================================
        # Expected: Complete analysis dataset from step02
        # Purpose: Extract variables for correlation and group comparison
        ...
        log(f"[INFO] Analysis variables extracted (n={len(df)})")
        log(f"[INFO] Theta range: {theta_scores.min():.3f} to {theta_scores.max():.3f}")
        log(f"[INFO] Rehearsal frequency range: {rehearsal_freq.min()} to {rehearsal_freq.max()}")
        log(f"[INFO] Mnemonic users: {mnemonic_use.sum()} / {len(mnemonic_use)} ({mnemonic_use.mean()*100:.1f}%)")

        # =========================================================================
        # STEP 2: Correlational Analysis (Rehearsal Frequency vs Theta)
        # =========================================================================
        # Analysis: Pearson correlation with bootstrap confidence intervals
        # Expected: Moderate positive correlation if rehearsal strategies effective

        log("[ANALYSIS] Computing Pearson correlation: rehearsal_frequency vs theta_all...")
        ...
        log(f"[SAVED] Correlation results: {correlation_path}")

        # =========================================================================
        # STEP 3: Group Comparison Analysis (Mnemonic Users vs Non-Users)
        # =========================================================================
        # Analysis: Independent t-test with D068 dual p-value reporting
        # Expected: Higher theta scores for mnemonic users

        log("[ANALYSIS] Comparing theta scores: mnemonic users vs non-users...")
        ...
        log(f"[GROUPS] Mnemonic users: n={n1}, Non-users: n={n2}")
        ...
        log(f"[MEANS] Mnemonic users: M={mean_mnemonic:.3f}, Non-users: M={mean_no_mnemonic:.3f}")
        ...
        log(f"[DEBUG] t_test_d068 returned keys: {list(ttest_result.keys())}")
        ...
        ci_lower_diff, ci_upper_diff = bootstrap_ttest_ci(mnemonic_group, no_mnemonic_group,
                                                         n_bootstrap=1000, seed=42)
        log(f"[BOOTSTRAP] 95% CI for mean difference: [{ci_lower_diff:.3f}, {ci_upper_diff:.3f}]")
        ...
        log(f"[SAVED] Group comparison results: {group_path}")

        # =========================================================================
        # STEP 4: Run Validation
        # =========================================================================
        # Validation: Check dual p-value format using validate_correlation_test_d068
        # Expected: Both uncorrected and Bonferroni-corrected p-values present

        log("[VALIDATION] Running correlation test validation...")
        ...
        try:
            ...
            log(f"[VALIDATION] Correlation results: {'PASS' if correlation_validation.get('valid', False) else 'FAIL'}")
            log(f"[VALIDATION] Group comparison results: {'PASS' if group_validation.get('valid', False) else 'FAIL'}")
        ...
        log("[SUMMARY] Analysis completed successfully:")
        ...
        log("[SUCCESS] Step 03 complete - correlational analysis with dual p-values")
        sys.exit(0)
