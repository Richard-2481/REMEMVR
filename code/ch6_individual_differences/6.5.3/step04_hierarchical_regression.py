...
PURPOSE:
Hierarchical regression with demographics block then strategy variables block
...
EXPECTED OUTPUTS:
  - results/ch7/7.5.3/data/step04_hierarchical_regression.csv
    ...
    Format: CSV with hierarchical regression results and incremental R-squared
    Expected rows: 2 blocks

  - results/ch7/7.5.3/data/step04_final_coefficients.csv
    Columns: ['predictor', 'coefficient', 'se', 't_statistic', 'p_uncorrected', 'p_bonferroni', 'cohens_f2']
    Format: CSV with final model coefficients and effect sizes
    Expected rows: ~6 predictors

VALIDATION CRITERIA:
  - regression_assumptions: Residuals normality, homoscedasticity, linearity
  - return_plots: Generate diagnostic plots for assumption checking

g_code REASONING:
- Approach: 2-block hierarchical regression (demographics -> strategy variables) with D068 dual p-values
- Why this approach: Controls for demographics before testing strategy effects, shows incremental variance
- Data flow: Analysis dataset → Block 1 (demographics) → Block 2 (+ strategy) → Model comparison
- Expected performance: ~30 seconds (regression fitting and diagnostics)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_regression.fit_hierarchical_regression with block structure
- Validation tool: validate_regression_assumptions for assumption checking
- Parameters: Block 1 = demographics, Block 2 = strategy variables
- Effect sizes: Cohen's f² for each predictor in final model
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        ...
        # =========================================================================
        # Expected: Complete analysis dataset with all variables for regression
        # Purpose: Hierarchical regression to test strategy effects controlling for demographics
        ...
        log(f"[OUTCOME] Theta scores - Mean: {y.mean():.3f}, SD: {y.std():.3f}")
        ...
        # =========================================================================
        # STEP 3: Fit Hierarchical Regression Models
        # =========================================================================
        ...
        try:
            ...
            log("[SUCCESS] Hierarchical regression completed via tools function")
            ...
            log(f"[WARNING] Tools function failed: {str(e)}, using custom implementation")
            
            # Custom hierarchical regression implementation
            log("[CUSTOM] Implementing hierarchical regression manually...")
            ...
            log(f"[CHANGE] ΔR² = {r_squared_change:.4f}, F({n_added_vars}, {df_resid}) = {f_change:.3f}, p = {p_change:.6f}")
        ...
        # =========================================================================
        # STEP 4: Calculate Effect Sizes (Cohen's f²)
        # =========================================================================
        ...
        log(f"[SAVED] Hierarchical regression results: {block_path}")
        ...
        log(f"[SAVED] Final model coefficients: {coef_path}")
        ...
        try:
            ...
            log("[VALIDATION] Regression assumptions check completed via tools function")
            ...
            if 'final_model' in locals():
                ...
                log("[VALIDATION] Skipping assumption checks - final_model not available")
                residuals = None
                fitted_values = None
