...
PURPOSE:
Bootstrap inference and cross-validation for robust statistical conclusions
...
EXPECTED INPUTS:
  - results/ch7/7.5.3/data/step02_analysis_dataset.csv
    ...
    Format: CSV with analysis dataset for sensitivity testing
    Expected rows: ~100

  - results/ch7/7.5.3/data/step05_outlier_analysis.csv
    Columns: ['UID', 'outlier_flag']
    Format: CSV with outlier identification for exclusion analysis
    Expected rows: ~100

EXPECTED OUTPUTS:
  - results/ch7/7.5.3/data/step06_bootstrap_results.csv
    Columns: ['predictor', 'original_coefficient', 'bootstrap_mean', 'bootstrap_se', 'CI_lower_percentile', 'CI_upper_percentile']
    Format: CSV with bootstrap confidence intervals for regression coefficients
    Expected rows: ~4 predictors

  - results/ch7/7.5.3/data/step06_outlier_sensitivity.csv
    Columns: ['analysis', 'n_cases', 'r_squared', 'rehearsal_coefficient', 'mnemonic_coefficient', 'rehearsal_p_value', 'mnemonic_p_value']
    Format: CSV with outlier sensitivity analysis results
    Expected rows: ~3 analyses (full, outliers excluded, extreme outliers only)

  - results/ch7/7.5.3/data/step06_cross_validation.csv
    Columns: ['fold', 'r_squared_cv', 'mean_cv_r_squared', 'se_cv_r_squared']
    Format: CSV with cross-validation results for generalization assessment
    Expected rows: ~5 folds

VALIDATION CRITERIA:
  - min_jaccard_threshold: Bootstrap stability with Jaccard >= 0.75
  - jaccard_values: Bootstrap replication stability measures

g_code REASONING:
- Approach: Bootstrap CIs (1000 reps) + outlier exclusion sensitivity + 5-fold cross-validation
- Why this approach: Provides robust inference, tests outlier impact, assesses generalization
- Data flow: Analysis dataset → Bootstrap inference → Outlier sensitivity → Cross-validation
- Expected performance: ~2-3 minutes (bootstrap computations and CV)

IMPLEMENTATION NOTES:
- Analysis tool: tools.analysis_regression.bootstrap_regression_ci with 1000 reps, seed=42
- Validation tool: validate_bootstrap_stability for checking replication consistency
- Parameters: Bootstrap n=1000, seed=42, alpha=0.05 for 95% CIs
- Sensitivity: Compare full sample vs outliers excluded vs extreme outliers only
"""
# =============================================================================
...
def outlier_sensitivity_analysis(df_full, df_outliers, predictor_vars, outcome_var):
    ...
    log("[SENSITIVITY] Analysis 1: Full sample")
    ...
    log(f"[DEBUG] Model params shape: {model_full.params.shape}")
    log(f"[DEBUG] Predictor vars: {predictor_vars}")
    ...
    try:
        ...
        log(f"[WARNING] Could not locate predictor indices: {str(e)}")
        rehearsal_idx = -1
        mnemonic_idx = -1
    ...
    if len(df_no_outliers) >= 20:  # Minimum sample size
        log(f"[SENSITIVITY] Analysis 2: Outliers excluded (n={len(df_no_outliers)})")
        ...
        log("[SENSITIVITY] Skipping outliers excluded analysis - insufficient sample size")
    ...
    if len(extreme_outlier_uids) >= 10:  # Only if enough extreme outliers
        log(f"[SENSITIVITY] Analysis 3: Extreme outliers only (n={len(extreme_outlier_uids)})")
        ...
        log("[SENSITIVITY] Skipping extreme outliers analysis - insufficient extreme outliers")
...
if __name__ == "__main__":
    try:
        log("[START] Step 06: Sensitivity analysis")

        # =========================================================================
        # STEP 1: Load Data Files
        # =========================================================================
        # Expected: Analysis dataset and outlier analysis from previous steps
        # Purpose: Bootstrap inference and sensitivity analysis for robust conclusions
        ...
        log("[LOAD] Loading outlier analysis...")
        outlier_path = RQ_DIR / "data" / "step05_outlier_analysis.csv"
        outlier_df = pd.read_csv(outlier_path)
        log(f"[LOADED] Outlier analysis ({len(outlier_df)} rows, {len(outlier_df.columns)} cols)")
        ...
        # =========================================================================
        # STEP 2: Bootstrap Regression Confidence Intervals
        # =========================================================================
        # Analysis: Bootstrap CIs with 1000 replications, seed=42
        # Expected: Robust confidence intervals for all regression coefficients
        ...
        try:
            ...
            log("[SUCCESS] Bootstrap CIs computed via tools function")
            
        except Exception as e:
            log(f"[WARNING] Tools bootstrap function failed: {str(e)}, using custom implementation")
        ...
        log("[BOOTSTRAP] Bootstrap confidence intervals:")
        ...
        log(f"[SAVED] Bootstrap results: {bootstrap_path}")

        # =========================================================================
        # STEP 3: Outlier Sensitivity Analysis
        # =========================================================================
        # Analysis: Compare full sample vs outliers excluded vs extreme outliers only
        # Purpose: Test whether outliers substantially affect conclusions

        log("[SENSITIVITY] Performing outlier sensitivity analysis...")
        ...
        log("[SENSITIVITY] Outlier sensitivity results:")
        ...
        log(f"[SAVED] Outlier sensitivity: {sensitivity_path}")

        # =========================================================================
        # STEP 4: Cross-Validation for Generalization Assessment
        # =========================================================================
        # Analysis: 5-fold cross-validation to assess model generalization
        # Purpose: Evaluate whether findings generalize beyond current sample
        ...
        log(f"[SAVED] Cross-validation results: {cv_path}")

        # =========================================================================
        # STEP 5: Run Validation
        # =========================================================================
        # Validation: Check bootstrap stability using validate_bootstrap_stability
        # Expected: Jaccard coefficient >= 0.75 for bootstrap stability

        log("[VALIDATION] Validating bootstrap stability...")
        ...
        try:
            ...
            log("[VALIDATION] Bootstrap stability validated via tools function")
        ...
        # =========================================================================
        # STEP 6: Summary of Sensitivity Analysis
        # =========================================================================
        
        log("[SUMMARY] Sensitivity analysis completed:")
        ...
        if bootstrap_coverage == len(predictor_vars) and r2_change < 0.05 and mean_cv_r2 > 0.1:
            log("[ROBUSTNESS] Results are ROBUST (stable bootstrap CIs, low outlier impact, good CV)")
        elif bootstrap_coverage >= len(predictor_vars) // 2:
            log("[ROBUSTNESS] Results are MODERATELY ROBUST (some concerns but interpretable)")
        else:
            log("[ROBUSTNESS] Results have CONCERNS (multiple stability issues)")

        log("[SUCCESS] Step 06 complete - comprehensive sensitivity analysis")
        sys.exit(0)
