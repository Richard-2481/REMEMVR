...
PURPOSE:
Merge strategy variables with theta scores and demographics, create analysis-ready dataset

EXPECTED INPUTS:
  - results/ch7/7.5.3/data/step00_merged_data.csv
    Columns: ['UID', 'theta_all', 'age', 'education', 'vr_exposure', 'sleep_hours']
    Format: CSV with merged theta and demographics data
    Expected rows: ~100

  - results/ch7/7.5.3/data/step01_strategy_variables.csv
    Columns: ['UID', 'rehearsal_frequency', 'mnemonic_use']
    Format: CSV with coded strategy variables
    Expected rows: ~100

EXPECTED OUTPUTS:
  - results/ch7/7.5.3/data/step02_descriptive_stats.csv
    Columns: ['variable', 'mean', 'std', 'min', 'max', 'n']
    Format: CSV with descriptive statistics
    Expected rows: ~7 variables
  ...
  - results/ch7/7.5.3/data/step02_analysis_dataset.csv
    ...
    Format: CSV with analysis-ready dataset
    Expected rows: ~100

VALIDATION CRITERIA:
  - expected_columns: All required analysis variables present
  - expected_rows: Approximately 100 participants with complete data
  - data_types: Numeric variables have appropriate types

g_code REASONING:
- Approach: Merge coded variables with outcomes, compute descriptives, convert education to numeric
- Why this approach: Need complete dataset with all variables for hierarchical regression
- Data flow: Strategy codes + merged data → Analysis dataset + descriptives
- Expected performance: ~30 seconds (data merging and descriptive statistics)

IMPLEMENTATION NOTES:
- Analysis tool: Custom processing for education conversion and descriptive statistics
- Validation tool: validate_data_format with adapted signature
- Parameters: Education ordinal-to-years mapping, outlier detection
- Quality checks: Normality tests, missing data patterns, distribution checks
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 02: Prepare analysis dataset")

        # =========================================================================
        # STEP 1: Load Data Files
        # =========================================================================
        # Expected: Merged data from step00 and strategy variables from step01
        # Purpose: Combine all variables needed for hierarchical regression

        log("[LOAD] Loading merged data...")
        merged_path = RQ_DIR / "data" / "step00_merged_data.csv"
        merged_df = pd.read_csv(merged_path)
        log(f"[LOADED] Merged data ({len(merged_df)} rows, {len(merged_df.columns)} cols)")
        
        log("[LOAD] Loading strategy variables...")
        strategy_path = RQ_DIR / "data" / "step01_strategy_variables.csv"
        strategy_df = pd.read_csv(strategy_path)
        log(f"[LOADED] Strategy variables ({len(strategy_df)} rows, {len(strategy_df.columns)} cols)")

        # =========================================================================
        # STEP 2: Merge Strategy Variables with Outcomes
        # =========================================================================
        # Join: strategy codes + theta scores + demographics on UID
        # Expected: Complete analysis dataset

        log("[MERGE] Merging strategy variables with outcomes...")
        analysis_df = merged_df.merge(
            strategy_df[['UID', 'rehearsal_frequency', 'mnemonic_use']], 
            on='UID', 
            how='inner'
        )
        log(f"[MERGED] Analysis dataset: {len(analysis_df)} participants")
        
        # Check for any data loss in merge
        if len(analysis_df) < len(merged_df):
            lost = len(merged_df) - len(analysis_df)
            log(f"[WARNING] Lost {lost} participants in strategy merge")

        # =========================================================================
        # STEP 3: Convert Education to Numeric Scale
        # =========================================================================
        # Tool: Ordinal-to-years mapping for UK education system
        # Purpose: Enable education as continuous predictor in regression

        log("[PROCESS] Converting education to numeric years...")
        ...
        log(f"[CONVERTED] Education years - Mean: {education_stats['mean']:.1f}, Range: {education_stats['min']:.0f}-{education_stats['max']:.0f}")
        
        # Show education distribution
        edu_dist = analysis_df.groupby('education')['education_numeric'].first().sort_values()
        log("[INFO] Education categories -> years mapping:")
        ...
        log(f"[SAVED] Descriptive statistics: {descriptives_path}")
        
        # Log key descriptive statistics
        log("[DESCRIPTIVES] Variable summary:")
        ...
        log(f"[FINAL] Analysis dataset: {final_n} complete cases")
        ...
        log(f"[SAVED] Analysis dataset: {output_path} ({len(final_df)} rows, {len(final_df.columns)} cols)")
        ...
        log("[VALIDATION] Running dataset validation...")
        ...
        log("[SUCCESS] Step 02 complete - analysis dataset prepared with descriptive statistics")
        sys.exit(0)
