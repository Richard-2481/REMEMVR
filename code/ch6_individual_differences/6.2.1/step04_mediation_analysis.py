...
"""
Step ID: step04
Step Name: Formal Mediation Analysis with Bootstrap
RQ: results/ch7/7.2.1
Generated: 2026-01-04

PURPOSE:
Formal mediation analysis testing whether cognitive tests (RAVLT, BVMT, RPM) 
mediate the relationship between age and REMEMVR VR memory performance. 
Computes mediation effect (c - c'), proportion mediated, and bootstrap 
significance test with effect size interpretation.

EXPECTED INPUTS:
  - results/ch7/7.2.1/data/step03_hierarchical_models.csv
    Columns: ['model', 'R2', 'R2_adj', 'F_stat', 'delta_R2', 'F_change', 'p_change', 'AIC']
    Format: Hierarchical regression results from Step 3
    Expected rows: 2 (Model 1: Age only, Model 2: Age + cognitive tests)
  - results/ch7/7.2.1/data/step01_analysis_dataset.csv
    Columns: ['UID', 'theta_all', 'SE', 'Age', 'RAVLT_T', 'BVMT_T', 'RPM_T', 'Age_std', 'RAVLT_T_std', 'BVMT_T_std', 'RPM_T_std']
    Format: Merged analysis dataset
    Expected rows: 100
...
EXPECTED OUTPUTS:
  - results/ch7/7.2.1/data/step04_mediation_analysis.csv
    ...
    Format: Mediation analysis results with bootstrap CIs
    Expected rows: 1

VALIDATION CRITERIA:
  - proportion_mediated: Valid range [0, 1] if total and direct effects same sign
  - ci_lower, ci_upper: Bootstrap confidence intervals exclude 0 for significance
  - effect_size_category: small (<0.25), medium (0.25-0.75), or large (>0.75)

g_code REASONING:
- Approach: Extract Age beta coefficients from Models 1 & 2, compute mediation effect
- Why this approach: Standard mediation analysis following Baron & Kenny framework
- Data flow: Step 3 models -> extract betas -> bootstrap significance -> effect size
- Expected performance: ~10-30 seconds (1000 bootstrap iterations)

IMPLEMENTATION NOTES:
- Analysis tool: bootstrap_regression_ci from tools.analysis_regression
- Validation tool: validate_numeric_range from tools.validation
- Parameters: 1000 bootstrap iterations, seed=42 for reproducibility
"""
# =============================================================================
...
#
# Examples:
#   CORRECT: data/step04_mediation_analysis.csv
#   CORRECT: data/step01_analysis_dataset.csv
#   WRONG:   results/mediation_analysis.csv  (wrong folder + no prefix)
#   WRONG:   data/mediation_analysis.csv     (missing step prefix)
#   WRONG:   logs/step04_mediation_results.csv (CSV in logs folder)
...
if __name__ == "__main__":
    try:
        log("[START] Step 4: Formal Mediation Analysis with Bootstrap")

        # =========================================================================
        # STEP 1: Load Input Data
        # =========================================================================
        # Expected: Step 3 hierarchical regression model results
        # Purpose: Extract Age beta coefficients for mediation calculation

        log("[LOAD] Loading hierarchical regression results from Step 3...")
        model_results = pd.read_csv(RQ_DIR / "data" / "step03_hierarchical_models.csv")
        log(f"[LOADED] step03_hierarchical_models.csv ({len(model_results)} rows, {len(model_results.columns)} cols)")

        log("[LOAD] Loading analysis dataset from Step 1...")
        dataset = pd.read_csv(RQ_DIR / "data" / "step01_analysis_dataset.csv")
        log(f"[LOADED] step01_analysis_dataset.csv ({len(dataset)} rows, {len(dataset.columns)} cols)")
        ...
        log(f"[IDENTIFIED] Model 1 (total effect): {model_1.iloc[0]['model']}")
        log(f"[IDENTIFIED] Model 2 (direct effect): {model_2.iloc[0]['model']}")

        # =========================================================================
        # STEP 2: Run Bootstrap Analysis for Beta Coefficient Extraction
        # =========================================================================
        # Tool: bootstrap_regression_ci to get precise Age coefficient estimates
        # What it does: Bootstrap confidence intervals for Age coefficients in both models
        # Expected output: DataFrames with coefficient estimates and CIs

        log("[ANALYSIS] Running bootstrap analysis for Model 1 (total effect = c path)...")
        ...
        log("[ANALYSIS] Running bootstrap analysis for Model 2 (direct effect = c' path)...")
        ...
        # =========================================================================
        # STEP 3: Formal Mediation Calculations
        # =========================================================================
        ...
        log("[MEDIATION] Computing formal mediation statistics...")

        # Mediation effect (indirect effect)
        mediation_effect = beta_total - beta_direct
        log(f"[MEDIATION] Mediation effect (c - c'): {mediation_effect:.4f}")

        # Proportion mediated (only meaningful if total and direct effects same sign)
        if beta_total == 0:
            proportion_mediated = np.nan
            log("[MEDIATION] Warning: Total effect is zero, proportion mediated undefined")
        else:
            proportion_mediated = mediation_effect / beta_total
            log(f"[MEDIATION] Proportion mediated: {proportion_mediated:.4f} ({proportion_mediated*100:.1f}%)")

        # Bootstrap significance test for mediation effect
        log("[BOOTSTRAP] Computing bootstrap confidence intervals for mediation effect...")
        ...
        log(f"[BOOTSTRAP] Mediation effect 95% CI: [{mediation_ci_lower:.4f}, {mediation_ci_upper:.4f}]")
        ...
        log(f"[SIGNIFICANCE] Mediation effect: {p_mediation} (95% CI {'excludes' if p_mediation == 'significant' else 'includes'} 0)")
        ...
        log(f"[INTERPRETATION] Effect size categories: small (<0.25), medium (0.25-0.75), large (>0.75)")

        # =========================================================================
        # STEP 4: Save Mediation Analysis Results
        # =========================================================================
        ...
        log("[SAVE] Saving mediation analysis results...")
        ...
        log(f"[SAVED] step04_mediation_analysis.csv ({len(mediation_results)} rows, {len(mediation_results.columns)} cols)")
        ...
        # Expected: Proportion mediated in reasonable bounds
        ...
        log("[INTERPRETATION] Mediation Analysis Summary:")
