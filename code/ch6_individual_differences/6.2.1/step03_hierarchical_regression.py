...
PURPOSE:
Hierarchical multiple regression to test VR scaffolding hypothesis.
Tests if age effect on VR theta_all scores is mediated by cognitive abilities.
...
EXPECTED INPUTS:
  - results/ch7/7.2.1/data/step01_analysis_dataset.csv
    ...
    Format: Merged and standardized analysis dataset
    Expected rows: ~100
...
EXPECTED OUTPUTS:
  ...
  - results/ch7/7.2.1/data/step03_model_diagnostics.csv
    ...
    Format: Assumption checks and diagnostics
    Expected rows: ~6

VALIDATION CRITERIA:
  - normality: Shapiro-Wilk p > 0.05 (residuals normally distributed)
  - homoscedasticity: Breusch-Pagan p > 0.05 (equal variances)
  - vif_threshold: All VIF < 5.0 (no multicollinearity)
  - cooks_d_threshold: All Cook's D < 4/n (no influential outliers)
  - alpha_bonferroni: 0.0125 for multiple comparison correction

g_code REASONING:
- Approach: Two-model hierarchical regression (Age-only vs Age+cognitive)
- Why this approach: Tests mediation hypothesis - Age should be significant in Model 1 but not Model 2
- Data flow: Load standardized data -> fit Model 1 (Age) -> fit Model 2 (Age+cognitive) -> compare via F-test
- Expected performance: ~seconds (simple regression models)

IMPLEMENTATION NOTES:
- Analysis tool: Custom hierarchical regression (signature mismatch in tools.analysis_regression)
- Validation tool: Custom diagnostics (signature mismatch in tools.analysis_extensions)
- Parameters: Model 1 uses Age_std only, Model 2 adds RAVLT_T_std, BVMT_T_std, RPM_T_std
- Key hypothesis: Age coefficient should drop to non-significance when cognitive tests added
"""
# =============================================================================
...
#
# Examples:
#   CORRECT: data/step03_hierarchical_models.csv
#   CORRECT: data/step03_model_diagnostics.csv
#   WRONG:   results/hierarchical_models.csv  (wrong folder + no prefix)
#   WRONG:   data/models.csv                  (missing step prefix)
#   WRONG:   logs/step03_results.csv          (CSV in logs folder)
...
def compute_regression_diagnostics_custom(model, X, y):
    ...
    try:
        ...
        log(f"[WARNING] Cook's Distance calculation failed: {e}")
        diagnostics.append({
            'test': 'outliers_max_cooks_d',
            'statistic': np.nan,
            'p_value': np.nan,
...
if __name__ == "__main__":
    try:
        log("[START] Step 03: Hierarchical Multiple Regression")

        # =========================================================================
        # STEP 1: Load Input Data
        # =========================================================================
        # Expected: Merged and standardized analysis dataset from step01
        # Purpose: Hierarchical regression to test age mediation by cognitive abilities
        ...
        log(f"[DATA] Response variable: theta_all (n={len(y)})")
        log(f"[DATA] Model 1 predictors: Age_std")
        log(f"[DATA] Model 2 additional predictors: RAVLT_T_std, BVMT_T_std, RPM_T_std, RAVLT_Pct_Ret_T_std, BVMT_Pct_Ret_T_std")

        # =========================================================================
        # STEP 2: Run Hierarchical Regression Analysis
        # =========================================================================
        # Tool: Custom hierarchical regression (original function had signature mismatch)
        # What it does: Fits Model 1 (Age only) then Model 2 (Age + cognitive tests)
        # Expected output: Model comparison with incremental R² and F-test
        ...
        log(f"[RESULTS] Model 1 R² = {model1_r2:.4f}")
        log(f"[RESULTS] Model 2 R² = {model2_r2:.4f}")
        log(f"[RESULTS] ΔR² = {delta_r2:.4f}")
        log(f"[RESULTS] F-change = {f_change:.4f}, p = {p_change:.4f}")
        ...
        log(f"[HYPOTHESIS] Age coefficient Model 1: β = {model1_age_coef:.4f}, p = {model1_age_p:.4f}")
        log(f"[HYPOTHESIS] Age coefficient Model 2: β = {model2_age_coef:.4f}, p = {model2_age_p:.4f}")
        log(f"[HYPOTHESIS] Bonferroni α = {alpha_bonferroni:.4f}")
        ...
        log(f"[HYPOTHESIS] Result: {hypothesis_result}")
        ...
        log(f"[SAVE] Saving hierarchical model results...")
        ...
        # =========================================================================
        # STEP 4: Run Regression Diagnostics
        # =========================================================================
        # Tool: Custom diagnostics (original function had signature mismatch)
        ...
        log("[VALIDATION] Running regression assumption checks...")
        ...
        log("[VALIDATION] Regression assumption checks:")
        for _, diag in diagnostics_df.iterrows():
            status = "VIOLATED" if diag['violated'] else "OK"
            if not pd.isna(diag['p_value']):
                log(f"[VALIDATION] {diag['test']}: {status} (statistic={diag['statistic']:.4f}, p={diag['p_value']:.4f})")
            else:
                log(f"[VALIDATION] {diag['test']}: {status} (statistic={diag['statistic']:.4f})")
        
        # Save diagnostics
        log(f"[SAVE] Saving regression diagnostics...")
        diagnostics_df.to_csv(RQ_DIR / "data/step03_model_diagnostics.csv", index=False, encoding='utf-8')
        log(f"[SAVED] step03_model_diagnostics.csv ({len(diagnostics_df)} rows, {len(diagnostics_df.columns)} cols)")
        ...
        if len(major_violations) > 0:
            log(f"[WARNING] {len(major_violations)} assumption violations detected:")
            for _, viol in major_violations.iterrows():
                log(f"[WARNING] - {viol['test']}: {viol['remedial_action']}")
        else:
            log("[VALIDATION] All regression assumptions satisfied")
