...
"""
Step ID: step05
Step Name: Cross-Validation Assessment
RQ: results/ch7/7.2.1
Generated: 2026-01-04

PURPOSE:
Implement 5-fold cross-validation to assess model generalizability for both 
Model 1 (Age only) and Model 2 (Age + cognitive tests). Evaluates out-of-sample
performance and detects overfitting through train-test R² gap analysis.
...
EXPECTED INPUTS:
  - results/ch7/7.2.1/data/step01_analysis_dataset.csv
    ...
    Format: CSV with 100 participants, standardized predictors
    Expected rows: ~100
...
EXPECTED OUTPUTS:
  - results/ch7/7.2.1/data/step05_cross_validation.csv
    ...
    Format: Cross-validation results for both models with overfitting detection
    Expected rows: ~2 (Model 1 and Model 2)

VALIDATION CRITERIA:
  - overfitting_threshold: 0.10 (train-test R² gap should be < 0.10)
  - R² range: 0.0 to 1.0 (valid range for coefficient of determination)

g_code REASONING:
- Approach: 5-fold cross-validation with sklearn.model_selection.KFold, random seed 42 for reproducibility
- Why this approach: Assesses model generalizability and overfitting, standard practice for regression validation
- Data flow: Load dataset → split into 5 folds → train models on 4 folds → test on 1 fold → aggregate metrics
- Expected performance: ~seconds (simple linear regression, small dataset)

IMPLEMENTATION NOTES:
- Analysis tool: Custom cross-validation (signature mismatch with tools.analysis_regression.cross_validate_regression)
- Validation tool: tools.validation.validate_numeric_range for R² bounds checking
- Parameters: n_folds=5, random_state=42, scoring_metrics=['mse', 'r2', 'mae']
- Special handling: Bootstrap confidence intervals for CV metrics, overfitting detection
"""
# =============================================================================
...
#   WRONG:   logs/step05_cv_results.csv       (CSV in logs folder)
...
if __name__ == "__main__":
    try:
        log("[START] Step 05: Cross-Validation Assessment")

        # =========================================================================
        # STEP 1: Load Input Data
        # =========================================================================
        # Expected: Analysis dataset from Step 1 with theta_all and standardized predictors
        # Purpose: Input for cross-validation assessment of Model 1 (Age only) and Model 2 (Age + cognitive)
        ...
        log(f"[EXTRACTED] y: {len(y)} observations")
        log(f"[EXTRACTED] Model 1 predictors: {list(X_model1.columns)}")
        log(f"[EXTRACTED] Model 2 predictors: {list(X_model2.columns)}")

        # =========================================================================
        # STEP 2: Run Cross-Validation for Both Models
        # =========================================================================
        # Tool: Custom cross-validation implementation (signature mismatch with tools function)
        # What it does: 5-fold CV with sklearn KFold, computes R², RMSE, MAE for each fold
        # Expected output: Mean and SD of performance metrics, overfitting detection
        ...
        log("[DONE] Cross-validation complete for both models")
        ...
        log("[ANALYSIS] Overfitting detection...")
        ...
        log(f"[OVERFITTING] Model 1 gap: {model1_gap:.4f}, Flag: {model1_overfitting}")
        log(f"[OVERFITTING] Model 2 gap: {model2_gap:.4f}, Flag: {model2_overfitting}")

        # =========================================================================
        # STEP 4: Bootstrap Confidence Intervals
        # =========================================================================
        # Tool: Custom bootstrap implementation for CV metrics
        # Purpose: Uncertainty quantification for cross-validation performance
        
        log("[BOOTSTRAP] Computing confidence intervals for CV metrics...")
        
        # Bootstrap CIs for Model 1
        log("[BOOTSTRAP] Model 1...")
        ...
        # =========================================================================
        # STEP 5: Save Cross-Validation Results
        # =========================================================================
        # These outputs will be used by: Step 6 (effect sizes), Step 9 (summary), rq_plots (visualization)
        ...
        log(f"[SAVED] step05_cross_validation.csv ({len(cv_results_df)} rows, {len(cv_results_df.columns)} cols)")
        ...
        # Expected: All R² values should be between 0.0 and 1.0 (valid range for coefficient of determination)
        ...
        if negative_r2:
            log(f"[WARNING] Negative R² detected - indicates model worse than baseline")
        
        # Log summary statistics
        log(f"[SUMMARY] Model 1 CV R²: {cv_results_model1['mean_scores']['r2']:.4f} ± {cv_results_model1['std_scores']['r2']:.4f}")
        log(f"[SUMMARY] Model 2 CV R²: {cv_results_model2['mean_scores']['r2']:.4f} ± {cv_results_model2['std_scores']['r2']:.4f}")
        log(f"[SUMMARY] Model 1 Overfitting: {model1_overfitting} (gap: {model1_gap:.4f})")
        log(f"[SUMMARY] Model 2 Overfitting: {model2_overfitting} (gap: {model2_gap:.4f})")
