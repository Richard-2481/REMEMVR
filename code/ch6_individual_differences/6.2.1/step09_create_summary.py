...
"""
Step ID: step09
Step Name: Create Analysis Summary
RQ: results/ch7/7.2.1
Generated: 2026-01-05

PURPOSE:
Generate a comprehensive summary of all analysis results from Steps 1-8,
providing a complete interpretation of the age moderation effects on the
test-VR relationship for thesis integration.
...
EXPECTED INPUTS:
  - results/ch7/7.2.1/data/step02_correlations.csv
    ...
    Format: Bivariate correlations with dual p-values (D068 compliance)
    Expected rows: ~6 correlations with theta_all
  - results/ch7/7.2.1/data/step03_hierarchical_models.csv
    Columns: ['model', 'R2', 'R2_adj', 'F_stat', 'delta_R2', 'F_change', 'p_change', 'AIC']
    Format: Model comparison results (Model 1: Age vs Model 2: Age+Cognitive)
  ...
  - results/ch7/7.2.1/data/step04_mediation_analysis.csv
    ...
    Format: Mediation analysis results showing suppression effects
    Expected rows: ~1 result
  - results/ch7/7.2.1/data/step05_cross_validation.csv
    Columns: ['model', 'cv_R2_mean', 'cv_R2_sd', 'cv_RMSE_mean', 'cv_RMSE_sd', 'cv_MAE_mean', 'cv_MAE_sd', 'train_R2', 'test_R2', 'overfitting_gap', 'overfitting_flag']
  ...
  - results/ch7/7.2.1/data/step06_effect_sizes.csv
    ...
    Format: Effect sizes and predictor importance
  ...
  - results/ch7/7.2.1/data/step07_power_analysis.csv
    ...
    Format: Post-hoc power analysis results
    Expected rows: ~4 test types

EXPECTED OUTPUTS:
  - results/ch7/7.2.1/data/step09_analysis_summary.txt
    Format: Comprehensive prose summary for thesis integration
    Expected content: Key findings, interpretation, statistical details, limitations
    Expected size: ~2-4 KB (comprehensive but concise)

VALIDATION CRITERIA:
  - validate_data_format: Check summary file format and completeness

g_code REASONING:
- Approach: Load all analysis results, synthesize key findings into interpretable narrative
- Why this approach: Provides thesis-ready summary emphasizing VR scaffolding hypothesis
- Data flow: CSV results -> statistical synthesis -> prose interpretation
- Expected performance: ~seconds (file I/O and text processing)

IMPLEMENTATION NOTES:
- Analysis tool: stdlib text operations (comprehensive summary generation)
- Validation tool: tools.validation.validate_data_format
- Key findings: Age suppression effect (negative -> positive), RPM strongest predictor (β=0.235)
- Special emphasis: Dual p-value reporting (D068), VR scaffolding hypothesis support
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 9: Create Analysis Summary")

        # =========================================================================
        # STEP 1: Load All Analysis Results
        # =========================================================================
        # Expected: Results from steps 2-7 containing complete statistical analysis
        # Purpose: Synthesize all findings into comprehensive interpretation

        log("[LOAD] Loading all analysis results...")
        
        # Load bivariate correlations (Step 2)
        correlations_df = pd.read_csv(RQ_DIR / "data/step02_correlations.csv")
        log(f"[LOADED] step02_correlations.csv ({len(correlations_df)} correlations)")
        
        # Load hierarchical regression results (Step 3)
        models_df = pd.read_csv(RQ_DIR / "data/step03_hierarchical_models.csv")
        log(f"[LOADED] step03_hierarchical_models.csv ({len(models_df)} models)")
        
        # Load mediation analysis results (Step 4)
        mediation_df = pd.read_csv(RQ_DIR / "data/step04_mediation_analysis.csv")
        log(f"[LOADED] step04_mediation_analysis.csv ({len(mediation_df)} results)")
        
        # Load cross-validation results (Step 5)
        cv_df = pd.read_csv(RQ_DIR / "data/step05_cross_validation.csv")
        log(f"[LOADED] step05_cross_validation.csv ({len(cv_df)} models)")
        
        # Load effect sizes (Step 6)
        effects_df = pd.read_csv(RQ_DIR / "data/step06_effect_sizes.csv")
        log(f"[LOADED] step06_effect_sizes.csv ({len(effects_df)} predictors)")
        
        # Load power analysis (Step 7)
        power_df = pd.read_csv(RQ_DIR / "data/step07_power_analysis.csv")
        log(f"[LOADED] step07_power_analysis.csv ({len(power_df)} tests)")

        # =========================================================================
        # STEP 2: Extract Key Findings
        # =========================================================================
        # What it does: Pull specific statistical results for narrative synthesis
        # Expected output: Key numbers ready for prose interpretation

        log("[EXTRACT] Extracting key statistical findings...")
        ...
        # =========================================================================
        # Tool: Custom text synthesis (comprehensive narrative generation)
        # What it does: Convert statistical results into thesis-ready interpretation
        # Expected output: Professional summary emphasizing VR scaffolding hypothesis

        log("[SYNTHESIS] Creating comprehensive analysis summary...")
...
"""
        ...
        log(f"[SAVED] step09_analysis_summary.txt ({len(summary_text)} characters)")
        ...
        # Expected: File created successfully with proper format
        ...
        if summary_path.exists():
            ...
            if file_size > 1000:  # Should be substantial summary
                log(f"[VALIDATION] Summary file size adequate: {file_size} bytes")
            else:
                log(f"[WARNING] Summary file may be too short: {file_size} bytes")
