...
if __name__ == "__main__":
    try:
        log("Step 00: Extract Random Effects from Ch5 5.1.4 Model-Averaged")
        # Load Ch5 5.1.4 Model-Averaged Random Effects
        log("Loading Ch5 5.1.4 model-averaged random effects...")
        ...
        log(f"Model-averaged random effects CSV")
        
        log(f"CSV shape: {random_effects_raw.shape}")
        log(f"Columns: {list(random_effects_raw.columns)}")
        ...
        log("CSV has expected columns for model-averaged random effects")
        # Process and Prepare Data for RQ 7.1.2
        log("Processing model-averaged random effects...")
        ...
        log(f"Random effects DataFrame: {random_effects_df.shape[0]} rows, {random_effects_df.shape[1]} cols")
        # Critical validation for RQ 7.1.2: Verify slope variation
        log("Checking slope variation for intercept vs slope analysis...")
        ...
        if slope_var < 0.001:  # Very small variance threshold
            log(f"Slope variance ({slope_var:.6f}) is very small - may not be sufficient for RQ 7.1.2")
        else:
            log(f"Random slopes have meaningful variation (variance={slope_var:.6f})")
            log(f"RQ 7.1.2 can proceed with intercept vs slope comparison")
        # Save processed random effects
        log(f"Saving processed random effects to {OUTPUT_FILE}...")
        random_effects_df.to_csv(OUTPUT_FILE, index=False)
        log(f"{random_effects_df.shape[0]} participants x {random_effects_df.shape[1]} columns")
        ...
        if len(random_effects_df) != 100:
            log(f"Expected 100 participants, got {len(random_effects_df)}")
        else:
            log("Participant count: 100")
        ...
        if intercept_in_range:
            log("All intercepts in range [-3.0, 3.0]")
        else:
            log("Some intercepts outside [-3.0, 3.0] range")
            
        if slope_in_range:
            log("All slopes in range [-2.0, 2.0]")
        else:
            log("Some slopes outside [-2.0, 2.0] range")
            
        if se_positive:
            log("All standard errors positive")
        else:
            log("Some standard errors not positive")
            
        # Check slope variance for RQ viability
        if slope_var >= 0.001:
            log(f"Slope variance ({slope_var:.6f}) sufficient for regression analysis")
            log(f"RQ 7.1.2 ready: Can compare cognitive test prediction of intercepts vs slopes")
        else:
            log(f"Low slope variance ({slope_var:.6f}) may limit power for slope prediction analysis")
        
        log("Step 00 complete - Model-averaged random effects extracted and validated")
        log(f"Source: Ch5 5.1.4 model-averaged random effects (accounting for model uncertainty)")
        log(f"Ready for Ch7 7.1.2 intercept vs slope prediction analysis")
        
    except Exception as e:
        log(f"{str(e)}")
        import traceback
        log(f"Full error details:")
        traceback.print_exc()
        raise