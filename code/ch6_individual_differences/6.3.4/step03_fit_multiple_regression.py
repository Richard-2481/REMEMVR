...
"""
Step ID: step03
Step Name: fit_multiple_regression
RQ: results/ch7/7.3.4
Generated: 2026-01-05T23:40:00Z

PURPOSE:
Fit three regression models to test differential DASS prediction patterns across
memory accuracy (theta), confidence, and calibration outcomes.

EXPECTED INPUTS:
  - results/ch7/7.3.4/data/step02_analysis_dataset.csv
    Columns: ['UID', 'z_Dep', 'z_Anx', 'z_Str', 'theta_accuracy', 'confidence', 'calibration']
    Format: CSV with merged analysis dataset
...
EXPECTED OUTPUTS:
  - results/ch7/7.3.4/data/step03_model_results.csv
    ...
    Format: Regression coefficients with dual p-values for all 3 models
    Expected rows: 9 coefficients total (3 models × 3 predictors each)
  - results/ch7/7.3.4/data/step03_model_diagnostics.csv
    Columns: ['model', 'shapiro_p', 'bp_p', 'dw_stat', 'min_vif', 'max_vif', 'r_squared', 'adj_r_squared']
    Format: Assumption test results for all 3 models
    Expected rows: 3 models

VALIDATION CRITERIA:
  - min_observations: 100 (sufficient sample size for regression)
...
g_code REASONING:
- Approach: Custom regression implementation using statsmodels.api.OLS due to signature mismatch in tools function
- Why this approach: Ch7 lessons learned show signature mismatches in regression tools, custom code more reliable
- Data flow: Load merged dataset → fit 3 separate OLS models → extract coefficients → bootstrap CIs → check assumptions
- Expected performance: ~2-3 minutes (bootstrap resampling with 1000 iterations)

IMPLEMENTATION NOTES:
- Analysis tool: Custom statsmodels.OLS implementation (tools.analysis_regression signature mismatch)
- Validation tool: tools.validation.validate_model_convergence
- Parameters: 3 models (accuracy/confidence/calibration), bootstrap 1000 iterations, seed=42
- Key insight: Following Ch7 lesson #9 - write custom regression when function signatures don't match
"""
# =============================================================================
...
#
# Examples:
#   CORRECT: data/step03_model_results.csv
#   CORRECT: data/step03_model_diagnostics.csv
#   WRONG:   results/model_results.csv  (wrong folder + no prefix)
#   WRONG:   data/model_results.csv     (missing step prefix)
...
if __name__ == "__main__":
    try:
        log("[START] Step 03: Fit Multiple Regression Models")

        # =========================================================================
        # STEP 1: Load Input Data
        # =========================================================================
        # Expected: Analysis dataset with DASS predictors and 3 outcomes
        # Purpose: Fit separate regression models for each outcome variable
        ...
        log(f"[LOADED] step02_analysis_dataset.csv ({len(input_data)} rows, {len(input_data.columns)} cols)")
        ...
        log(f"[CHECK] All required columns present: {required_predictors + required_outcomes}")
        ...
        log(f"[SETUP] Predictors: {required_predictors}")
        log(f"[SETUP] Models: {list(outcomes.keys())}")
        log(f"[SETUP] Sample size: N = {len(X)}")

        # =========================================================================
        # STEP 2: Run Regression Analysis (Custom Implementation)
        # =========================================================================
        # Tool: Custom statsmodels.OLS implementation (signature mismatch in tools function)
        # What it does: Fits 3 separate OLS models with bootstrap CIs and diagnostics
        # Expected output: Model results with coefficients, p-values, and assumption tests
        ...
        for model_config in models_config:
            ...
            log(f"[MODEL] Fitting {model_name} (outcome: {outcome_col})...")
            ...
            log(f"[DONE] {model_name}: R² = {model_results['rsquared']:.3f}, F-test p = {model_results['f_pvalue']:.4f}")
        
        log(f"[ANALYSIS] All 3 models fitted successfully")
        ...
        log(f"[SAVED] step03_model_results.csv ({len(results_df)} rows, {len(results_df.columns)} cols)")
        ...
        for model_name, model_obj in model_objects.items():
            log(f"[VALIDATION] Checking {model_name}...")
            ...
            if isinstance(validation_result, dict):
                for key, value in validation_result.items():
                    log(f"[VALIDATION] {model_name} {key}: {value}")
            else:
                log(f"[VALIDATION] {model_name}: {validation_result}")

        # Summary validation checks
        log("[VALIDATION] Summary checks:")
        log(f"[CHECK] All 3 models converged: {len(model_objects) == 3}")
        log(f"[CHECK] 9 coefficient estimates: {len(results_df) == 9}")
        log(f"[CHECK] Bootstrap CIs computed: {all(~pd.isna(results_df['ci_lower']))}")
        log(f"[CHECK] VIF < 5.0 check: {all(results_df['vif'] < 5.0)} (max VIF = {results_df['vif'].max():.2f})")
        log(f"[CHECK] Assumption tests completed: {len(diagnostics_df) == 3}")
        
        # Report coefficient summary
        log("[SUMMARY] Model performance:")
        for model_name in ['accuracy_model', 'confidence_model', 'calibration_model']:
            ...
            log(f"[SUMMARY] {model_name}: R² = {r2:.3f}, Adj R² = {adj_r2:.3f}")
