...
def outlier_analysis(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for bvmt_col, label in [('bvmt_total', 'total'), ('bvmt_pct_ret', 'pct_ret')]:
        ...
        log(f"[OUTLIER] {label}: Found {len(outliers_where)} outliers using threshold {outlier_threshold}")
        ...
        if len(outliers_where) > 0:
            ...
            log(f"[OUTLIER] {label}: Dataset size after outlier removal: {len(df_clean)} (removed {len(outliers_where)})")
        else:
            df_clean = df.copy()
            log(f"[OUTLIER] {label}: No outliers detected")
...
def alternative_correlation_methods(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for bvmt_col, label in [('bvmt_total', 'total'), ('bvmt_pct_ret', 'pct_ret')]:
        ...
        log(f"[METHODS] {label} Spearman Where: r={spear_where.correlation:.3f}, p={spear_where.pvalue:.3f}")
        log(f"[METHODS] {label} Kendall Where: tau={kendall_where.correlation:.3f}, p={kendall_where.pvalue:.3f}")
...
def power_analysis(df: pd.DataFrame, alpha: float = 0.00179) -> Dict[str, Any]:
    ...
    for bvmt_col, label in [('bvmt_total', 'total'), ('bvmt_pct_ret', 'pct_ret')]:
        ...
        log(f"[POWER] {label}: Power Where={power_where:.3f}, Power What={power_what:.3f}")
