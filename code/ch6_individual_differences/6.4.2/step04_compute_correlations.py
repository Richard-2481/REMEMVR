...
"""
Step ID: step04
Step Name: step04_compute_correlations
RQ: results/ch7/7.4.2
Generated: 2026-01-06

PURPOSE:
Compute r(BVMT, Where) and r(BVMT, What) with bootstrap 95% confidence intervals.
Tests which domain (Where vs What) has stronger correlation with visuospatial memory (BVMT).

EXPECTED INPUTS:
- results/ch7/7.4.2/data/step03_analysis_dataset.csv
  Columns: ["UID", "bvmt_total", "Where_mean", "What_mean"]
  Format: CSV with complete analysis dataset (domain theta scores + BVMT)
...
- results/ch7/7.4.2/data/step04_correlations.csv
  ...
  Format: CSV with bootstrap correlation results and confidence intervals
  Expected rows: 4 correlations (BVMT_Where, BVMT_What, BVMTret_Where, BVMTret_What)
...
g_code REASONING:
- Approach: Use bootstrap_correlation_ci from tools.bootstrap for robust CI estimation
- Why this approach: Bootstrap provides non-parametric confidence intervals, better for small samples
- Data flow: Load merged dataset -> extract variables -> compute 2 correlations -> save results
- Expected performance: ~10-30 seconds per correlation (1000 bootstrap iterations)
...
- Key lesson: Function returns dict with 'r' key (not 'correlation') based on gcode_lessons.md
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        log("[START] Step 04: Compute correlations with bootstrap CIs")

        # =========================================================================
        # STEP 1: Load Input Data
        # =========================================================================
        # Expected: Analysis dataset with domain theta scores and BVMT scores
        # Purpose: Compute correlations between visuospatial memory and domain abilities
        ...
        log(f"[INFO] Dataset summary: {len(analysis_df)} participants")

        # =========================================================================
        # STEP 2: Run Analysis Tool - Bootstrap Correlations
        # =========================================================================
        # Tool: bootstrap_correlation_ci
        # What it does: Computes Pearson correlation with bootstrap confidence intervals
        # Expected output: Dict with 'r', 'ci_lower', 'ci_upper', 'se', 'bootstrap_samples'
        ...
        for corr_info in correlations_to_compute:
            log(f"[ANALYSIS] Computing {corr_info['name']} correlation...")
            log(f"[ANALYSIS] Variables: {corr_info['x_var']} vs {corr_info['y_var']}")
            ...
            log(f"[ANALYSIS] Clean data: {len(x_clean)} valid pairs ({len(x_data) - len(x_clean)} removed)")
            ...
            log(f"[DONE] {corr_info['name']}: r = {correlation_coef:.4f}, CI = [{ci_lower:.4f}, {ci_upper:.4f}]")
            ...
            log(f"[INFO] {corr_info['name']} effect size: {effect_size} (|r| = {abs_r:.3f})")
        ...
        log(f"[SAVE] Saving step04_correlations.csv...")
        ...
        log(f"[SAVED] step04_correlations.csv ({len(correlation_df)} rows, {len(correlation_df.columns)} cols)")
        
        # Log summary of results
        log("[SUMMARY] Correlation results:")
        ...
        # Expected: All correlations valid, CIs contain point estimates, CI bounds sensible
        ...
        for _, row in correlation_df.iterrows():
            if row['n'] < 50:
                log(f"[WARNING] {row['correlation']}: Small sample size (n = {row['n']})")
        ...
        if validation_issues:
            ...
            log("[VALIDATION] All correlation results passed validation checks")

        log("[SUCCESS] Step 04 complete")
        log(f"[NEXT] Ready for Step 05: Steiger's Z-test to compare r(BVMT,Where) vs r(BVMT,What)")
        sys.exit(0)
