...
PURPOSE:
Check regression assumptions and identify remedial actions for the full regression model.
Computes comprehensive regression diagnostics including VIF, Cook's D, assumption tests,
and participant-level diagnostic values for identifying violations and suggesting remedial actions.

EXPECTED INPUTS:
  - results/ch7/7.5.2/data/step01_analysis_dataset.csv
    Columns: ["UID", "theta_all", "dass_dep", "dass_anx", "dass_str", "age", "nart_score"]
    Format: Complete cases analysis dataset from Step 1
    Expected rows: ~97 cases

EXPECTED OUTPUTS:
  - results/ch7/7.5.2/data/step05_model_diagnostics.csv
    Columns: ["predictor", "VIF", "condition_number", "breusch_pagan_p", "durbin_watson"]
    Format: Overall model diagnostics
    Expected rows: ~5 predictors

  - results/ch7/7.5.2/data/step05_residual_analysis.csv
    Columns: ["observation", "fitted", "residual", "standardized_residual", "leverage", "cooks_d"]
    Format: Participant-level diagnostic values
    Expected rows: ~97 observations

  - results/ch7/7.5.2/data/step05_assumption_checks.txt
    Format: Assumption test results and remedial action recommendations
    Content: VIF violations, normality tests, homoscedasticity, autocorrelation
...
VALIDATION CRITERIA:
  ...
  - cooks_d_threshold: 0.041 (4/97)

g_code REASONING:
- Approach: Fit full regression model (theta_all ~ age + nart_score + dass_dep + dass_anx + dass_str) 
  then compute comprehensive diagnostics using statsmodels and scipy tests
- Why this approach: Need to identify assumption violations before interpreting results from Steps 3-4,
  especially given known high multicollinearity detected in Step 4
- Data flow: Load dataset → fit OLS model → compute VIF, Cook's D, residuals → run diagnostic tests → 
  identify violations → suggest remedial actions
- Expected performance: ~30 seconds (simple OLS regression + diagnostic calculations)

IMPLEMENTATION NOTES:
- Analysis tool: Custom implementation due to signature mismatch in tools.analysis_regression.compute_regression_diagnostics
- Validation tool: tools.validation.validate_lmm_assumptions_comprehensive (signature mismatch but will use OLS model)
- Parameters: VIF threshold 5.0, Cook's D threshold 0.041 (4/N), alpha 0.05 for all tests
- Critical: Document high multicollinearity from Step 4 in remedial actions
"""
# =============================================================================
...
if __name__ == "__main__":
    try:
        ...
        # =========================================================================
        # Expected: Complete cases analysis dataset from Step 1
        # Purpose: Fit full regression model for comprehensive diagnostic testing
        ...
        log(f"[VERIFIED] All required columns present: {required_cols}")

        # =========================================================================
        # STEP 2: Prepare Regression Model Data
        # =========================================================================
        ...
        log("[PREPARE] Setting up regression model data...")
        ...
        log(f"[PREPARED] Sample size: {len(X)} observations")
        ...
        log("[VERIFIED] No missing data in regression variables")

        # =========================================================================
        # STEP 3: Fit Full Regression Model
        # =========================================================================
        # Tool: Custom OLS implementation (due to signature mismatch in tools.analysis_regression.compute_regression_diagnostics)
        # What it does: Fit ordinary least squares regression with constant term
        # Expected output: Fitted model for diagnostic computation
        ...
        log("[DONE] Regression model fitted successfully")
        log(f"[MODEL] R-squared: {full_model.rsquared:.4f}")
        log(f"[MODEL] Adj. R-squared: {full_model.rsquared_adj:.4f}")
        log(f"[MODEL] F-statistic: {full_model.fvalue:.4f} (p={full_model.f_pvalue:.6f})")

        # =========================================================================
        # STEP 4: Compute Model-Level Diagnostics
        # =========================================================================
        ...
        log("[DIAGNOSTICS] Computing model-level diagnostics...")
        ...
        log(f"[VIF] Computed VIF for {len(vif_data)} predictors")
        ...
        log(f"[CONDITION] Condition number: {condition_number:.2f}")
        ...
        log(f"[DIAGNOSTICS] Model-level diagnostics computed for {len(model_diagnostics_df)} predictors")

        # =========================================================================
        # STEP 5: Compute Observation-Level Diagnostics
        # =========================================================================
        # Cook's D, leverage, standardized residuals for outlier detection
        
        log("[DIAGNOSTICS] Computing observation-level diagnostics...")
        ...
        log(f"[DIAGNOSTICS] Observation-level diagnostics computed for {len(residual_analysis)} observations")
        ...
        log("[ASSUMPTIONS] Running assumption tests...")
        ...
        log(f"[ASSUMPTIONS] {len(assumption_violations)} potential violations identified")
        ...
        log("[SAVE] Saving model diagnostics...")
        model_diagnostics_df.to_csv(RQ_DIR / "data/step05_model_diagnostics.csv", index=False, encoding='utf-8')
        log(f"[SAVED] step05_model_diagnostics.csv ({len(model_diagnostics_df)} rows, {len(model_diagnostics_df.columns)} cols)")

        log("[SAVE] Saving residual analysis...")
        residual_analysis.to_csv(RQ_DIR / "data/step05_residual_analysis.csv", index=False, encoding='utf-8')
        log(f"[SAVED] step05_residual_analysis.csv ({len(residual_analysis)} rows, {len(residual_analysis.columns)} cols)")
        ...
        log("[VALIDATION] Attempting validation...")
        ...
        try:
            ...
            # Expected: validation function may not be compatible with OLS results
            log(f"[VALIDATION WARNING] Validation function failed (expected with OLS model): {str(e)}")
            log("[VALIDATION] Proceeding with manual diagnostic assessment")
