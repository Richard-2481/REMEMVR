...
if __name__ == "__main__":
    try:
        log("Step 01: Extract and Prepare Cognitive Test Data")
        log(f"RQ Directory: {RQ_DIR}")
        log(f"Output will be saved to: {OUTPUT_FILE}")
        # Load participant data with cognitive tests
        ...
        log(f"Loaded cognitive test data for {len(cognitive_df)} participants")
        log(f"Columns extracted: {list(cognitive_df.columns)}")
        # Check what we got and compute derived scores if needed
        log("Computing derived scores...")
        
        # Check if RAVLT_Total already exists or needs to be computed
        if 'RAVLT_Total' not in cognitive_df.columns:
            log("Computing RAVLT_Total from trial scores...")
            ...
            if len(ravlt_trial_cols) == 5:
                log(f"Summing RAVLT trials 1-5: {ravlt_trial_cols}")
                cognitive_df['RAVLT_Total'] = cognitive_df[ravlt_trial_cols].sum(axis=1)
            else:
                log("RAVLT trial columns not found, checking for RAVLT_Total...")
        ...
        for test in ['RAVLT', 'BVMT', 'NART', 'RPM']:
            ...
            log(f"Found {test} columns: {matching}")
            test_cols.extend(matching)
        # Select and rename columns for analysis
        log("Selecting and renaming cognitive test columns...")
        ...
        if bvmt_col:
            ...
            log(f"Using {bvmt_col} for BVMT")
        else:
            log("BVMT total score not found")
        ...
        if nart_col:
            ...
            log(f"Using {nart_col} for NART")
        else:
            log("NART score not found")
        ...
        if rpm_col:
            ...
            log(f"Using {rpm_col} for RPM")
        else:
            log("RPM score not found")
        # Convert to T-scores (M=50, SD=10)
        log("Converting raw scores to T-scores (M=50, SD=10)...")
        ...
        for test in ['RAVLT', 'BVMT', 'NART', 'RPM']:
            ...
            if raw_col in final_df.columns:
                ...
                if len(valid_scores) > 0:
                    ...
                    log(f"{test}: Mean={mean:.2f}, SD={sd:.2f}, N={len(valid_scores)}")
                    ...
                    if sd > 0:
                        ...
                        log(f"{test}_T: Mean={t_mean:.2f}, SD={t_sd:.2f}")
                    else:
                        log(f"{test} has zero variance, cannot compute T-scores")
                        final_df[t_col] = 50  # Set to mean if no variance
                else:
                    log(f"No valid {test} scores found")
        # Handle missing data
        ...
        for col in final_df.columns:
            if col != 'UID':
                ...
                if n_missing > 0:
                    log(f"{col}: {n_missing} missing values ({n_missing/len(final_df)*100:.1f}%)")
        
        # Count complete cases
        t_cols = [col for col in final_df.columns if col.endswith('_T')]
        complete_cases = final_df[t_cols].notna().all(axis=1).sum()
        log(f"Complete cases (all 4 T-scores): {complete_cases}/{len(final_df)}")
        # Save output
        log(f"Saving cognitive test T-scores to {OUTPUT_FILE}...")
        ...
        log(f"{len(output_df)} participants x {len(output_cols)} columns")
        # Validation
        log("Verifying output file...")
        
        # Reload and check
        check_df = pd.read_csv(OUTPUT_FILE)
        log(f"Output file has {len(check_df)} rows, {len(check_df.columns)} columns")
        log(f"Columns: {list(check_df.columns)}")
        ...
        for test in ['RAVLT', 'BVMT', 'NART', 'RPM']:
            ...
            if t_col in check_df.columns:
                ...
                log(f"{t_col}: Mean={mean:.2f} (target=50), SD={sd:.2f} (target=10)")
        
        log("Step 01 complete - Cognitive test T-scores extracted and saved")
